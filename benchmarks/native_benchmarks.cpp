...
namespace
{
    ...
    std::size_t projected_serialized_sample_count(const ballistics::Trajectory& trajectory)
    {
    ...
    {
        const auto index = static_cast<std::size_t>(std::ceil(fraction * static_cast<double>(sorted_values.size())) - 1.0);
        return sorted_values[std::min(index, sorted_values.size() - 1)];
    }

    BenchmarkResult benchmark(std::string id, std::size_t iterations, const std::function<BenchmarkObservation()>& operation)
    {
    ...
    BenchmarkObservation trajectory_observation(const ballistics::Trajectory& trajectory)
    {
        const auto& sample = trajectory.samples.back();
        return { trajectory.covered_distance_m + sample.ground_speed_mps + sample.time_s + sample.position_m.y,
            true,
    ...
    void write_results(std::ostream& output, const std::vector<BenchmarkResult>& results)
    {
        output
            << std::setprecision(15) << "{\n"
            << "  \"schemaVersion\": 1,\n"
        ...
        for (std::size_t index = 0; index < results.size(); ++index)
        {
            ...
            output
                ...
                << "      \"guard\": " << result.guard << ",\n"
                << "      \"integrationDiagnosticsAvailable\": " << (result.integration_diagnostics_available ? "true" : "false") << ",\n"
                ...
                << "      \"estimatedRetainedBytesPerIteration\": " << result.estimated_retained_bytes_per_iteration << ",\n"
                << "      \"serializedSamplesPerIteration\": " << result.serialized_samples_per_iteration << "\n"
...
int main(int argc, char** argv)
{
    ...
    for (std::size_t index = 0; index < 3; ++index)
    {
        ...
        load.definition.drag = ReferenceBcDrag { ReferenceDragCurve::g7, ConstantBallisticCoefficient { 0.20 + 0.05 * static_cast<double>(index) } };
        nine_loads.push_back(load);
    }

    auto calibration_truth = built_ins[3];
    calibration_truth.definition.drag = ReferenceBcDrag { ReferenceDragCurve::g7, ConstantBallisticCoefficient { 0.25 } };
    ...
    for (const double distance_m : { 100.0, 200.0, 300.0, 400.0, 500.0 })
    {
        ...
        calibration_observations.push_back({ distance_m, sample->ground_speed_mps, 1.0, distance_m < 500.0 ? ObservationRole::calibration : ObservationRole::holdout });
    }
    auto calibration_guess = calibration_truth;
    calibration_guess.definition.drag = ReferenceBcDrag { ReferenceDragCurve::g7, ConstantBallisticCoefficient { 0.20 } };

    const auto uncertainty_baseline = integrate_zeroed_trajectory(built_ins[3], atmosphere, 500, 100, 0.04);
    ...
    {
        results.push_back(benchmark("six-builtins-" + std::to_string(static_cast<int>(distance_m)) + "m",
            iterations,
            [&]
            {
                ...
                {
                    const auto observation = trajectory_observation(integrate_trajectory(load, atmosphere, distance_m));
    ...
    results.push_back(benchmark("nine-loads-2000m",
        iterations,
        [&]
        {
            BenchmarkObservation total { 0, true, 0, 0, 0, 0, 0 };
            ...
            {
                const auto observation = trajectory_observation(integrate_trajectory(load, atmosphere, 2000));
                total.guard += observation.guard;
                total.accepted_steps += observation.accepted_steps;
                total.rejected_steps += observation.rejected_steps;
                total.retained_samples += observation.retained_samples;
    ...
    results.push_back(benchmark("nine-loads-2000m-full-calculation",
        iterations,
        [&]
        {
            BenchmarkObservation total { 0, true, 0, 0, 0, 0, 0 };
            for (const auto& load : nine_loads)
            {
                ...
                const auto zeroed = integrate_zeroed_trajectory(load, atmosphere, 2000, zero_range_m, sight_height_m);
                ...
                total.guard += observation.guard + mpbr.zero_m + mpbr.mpbr_m + events.analyzed_distance_m + static_cast<double>(validity.status == DragValidityStatus::within_domain);
                total.accepted_steps += observation.accepted_steps;
                total.rejected_steps += observation.rejected_steps;
                total.retained_samples += observation.retained_samples;
                total.estimated_retained_bytes += observation.estimated_retained_bytes;
    ...
    results.push_back(benchmark("constant-g7-calibration",
        iterations,
        [&]
        {
            const auto result = calibrate_reference_ballistic_coefficient(calibration_guess, atmosphere, calibration_observations, BcFitKind::constant);
            return BenchmarkObservation { result.estimates.front().ballistic_coefficient + result.calibration_rmse_mps, false, 0, 0, 0, 0, 0 };
        }));
    results.push_back(benchmark("three-input-uncertainty",
        iterations,
        [&]
        {
            const auto result = propagate_trajectory_uncertainty(built_ins[3], atmosphere, uncertainty_baseline, 500, 100, 0.04, 1.0, uncertainty, uncertainty_ranges);
            return BenchmarkObservation { result.samples.back().speed_standard_deviation_mps + result.samples.back().path_standard_deviation_m, false, 0, 0, 0, 0, 0 };
        }));
