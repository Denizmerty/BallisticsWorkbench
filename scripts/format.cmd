...
for /r "%~dp0..\src" %%F in (*.cpp *.hpp) do "%CLANG_FORMAT%" -i --style=file "%%F"
for /r "%~dp0..\tests" %%F in (*.cpp *.hpp) do "%CLANG_FORMAT%" -i --style=file "%%F"

call npm exec prettier -- --write "src/Ballistics.Desktop/**/*.{ts,tsx,cjs,css,json}" "vite.config.ts" "index.html" "README.md" "CHANGELOG.md" "docs/**/*.md" ".github/**/*.{yml,yaml}" "*.json"
if errorlevel 1 exit /b %errorlevel%

call npm exec prettier -- --write --parser html "assets/**/*.svg"
exit /b %errorlevel%
