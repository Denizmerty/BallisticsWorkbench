...
namespace ballistics::json
{
    ...
    {
    public:
        ParseError(const std::string& message, std::size_t offset) : std::runtime_error(message), offset_(offset)
        {
        }
    ...
    private:
        ...
        [[nodiscard]] Value::Array parse_array(std::size_t depth)
        {
            expect('[');
            skip_whitespace();
            Value::Array values;
        ...
        [[nodiscard]] Value::Object parse_object(std::size_t depth)
        {
            expect('{');
            skip_whitespace();
            Value::Object values;
            ...
            while (position_ < source_.size())
            {
                ...
                case 'u':
                {
                    ...
                    if (high >= 0xd800 && high <= 0xdbff)
                    {
                        ...
                        codepoint = 0x10000u + ((static_cast<std::uint32_t>(high) - 0xd800u) << 10u) + (static_cast<std::uint32_t>(low) - 0xdc00u);
                    }
