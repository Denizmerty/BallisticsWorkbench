...
namespace ballistics::cli
{
    namespace
    {
        ...
        void require_range(double value, double minimum, double maximum, const char* label)
        {
            ...
            {
                throw std::invalid_argument(std::string(label) + " must be between " + std::to_string(minimum) + " and " + std::to_string(maximum));
            }
        }

    } // namespace

    ballistics::protocol::Request legacy_request(int argc, char** argv)
    {
        ...
        require_range(scenario.shotgun.muzzle_velocity_multiplier, .75, 1.25, "Shotgun velocity multiplier");
        require_range(scenario.rifle.muzzle_velocity_multiplier, .75, 1.25, "Rifle velocity multiplier");
        ...
        if (!std::isfinite(twist) || !std::isfinite(length) || !std::isfinite(bullet_diameter) || twist < 0 || length < 0 || bullet_diameter < 0)
        {
        ...
        else
        {
            ...
            if (!std::isfinite(ballistic_coefficient) || ballistic_coefficient <= 0 || ballistic_coefficient > 2)
            {
                throw std::invalid_argument("Custom ballistic coefficient must be positive and at most 2");
            }
            diameter = 0;
            density = 0;
