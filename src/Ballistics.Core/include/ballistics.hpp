...
namespace ballistics
{
    ...
    using BallisticCoefficientDefinition = std::variant<ConstantBallisticCoefficient, BandedBallisticCoefficient>;
    ...
    struct Atmosphere
    {
        ...
        // convention).
        [[nodiscard]] static Atmosphere create(double temperature_c, double station_pressure_hpa, double relative_humidity_percent, double headwind_mps, double crosswind_mps = 0.0);
    };
    ...
    struct Environment
    {
        ...
        [[nodiscard]] Vec3 coriolis_acceleration(const Vec3& ground_velocity_mps, double height_m) const;
    };
    ...
    struct TrajectoryEvents
    {
        ...
        TrajectoryEventStatus ground_intersection_status { TrajectoryEventStatus::baseline_unavailable };
    ...
    [[nodiscard]] double altimeter_setting_to_station_pressure_hpa(double altimeter_setting_hpa, double geometric_altitude_m);

    [[nodiscard]] double station_pressure_to_altimeter_setting_hpa(double station_pressure_hpa, double geometric_altitude_m);
    ...
    [[nodiscard]] double line_of_sight_path_m(const TrajectorySample& sample, double sight_height_m, double sight_line_elevation_rad);
    ...
    [[nodiscard]] double drag_retardation_mps2(double relative_speed_mps, const Projectile& projectile, const Atmosphere& atmosphere);

    [[nodiscard]] double effective_ballistic_coefficient(const Projectile& projectile, double airspeed_mps);

    [[nodiscard]] AerodynamicDiagnostics aerodynamic_diagnostics(const Projectile& projectile, const Vec3& ground_velocity_mps, const Atmosphere& atmosphere);

    [[nodiscard]] Trajectory integrate_trajectory(const Projectile& projectile, const Atmosphere& atmosphere, double maximum_distance_m, double muzzle_velocity_multiplier = 1.0, double dx_m = 0.25);

    [[nodiscard]] Trajectory integrate_trajectory(
        const Projectile& projectile, const Atmosphere& atmosphere, double maximum_distance_m, double muzzle_velocity_multiplier, double output_interval_m, const SolverConfiguration& configuration);

    [[nodiscard]] Trajectory integrate_trajectory(
        const Projectile& projectile, const Environment& environment, double maximum_distance_m, double muzzle_velocity_multiplier, double output_interval_m, const SolverConfiguration& configuration);

    [[nodiscard]] ZeroedTrajectory integrate_shot_scenario(const Projectile& projectile, const ShotScenario& scenario);
    ...
    [[nodiscard]] Trajectory integrate_trajectory_legacy(
        const Projectile& projectile, const Atmosphere& atmosphere, double maximum_distance_m, double muzzle_velocity_multiplier = 1.0, double dx_m = 0.25);

    [[nodiscard]] ZeroedTrajectory integrate_zeroed_trajectory(const Projectile& projectile, const Atmosphere& atmosphere, double maximum_distance_m, double zero_range_m, double sight_height_m,
        double muzzle_velocity_multiplier = 1.0, double output_interval_m = 0.25, const SolverConfiguration& configuration = SolverConfiguration {});

    [[nodiscard]] TrajectoryEvents analyze_trajectory_events(const ZeroedTrajectory& zeroed, double sight_height_m, double sight_line_elevation_rad = 0.0);

    [[nodiscard]] MpbrResult compute_mpbr(const Trajectory& trajectory, double vital_zone_m, double sight_height_m = 0.0);

    [[nodiscard]] MpbrResult compute_native_mpbr(const Projectile& projectile, const Atmosphere& atmosphere, double maximum_distance_m, double vital_zone_m, double sight_height_m = 0.0,
        double muzzle_velocity_multiplier = 1.0, double output_interval_m = 0.25, const SolverConfiguration& configuration = SolverConfiguration {});

    [[nodiscard]] MpbrResult compute_scenario_mpbr(const Projectile& projectile, const ShotScenario& scenario);

    [[nodiscard]] DragValidity evaluate_drag_validity(const Projectile& projectile, const Trajectory& trajectory, double maximum_distance_m);

    [[nodiscard]] BcCalibrationResult calibrate_reference_ballistic_coefficient(const Projectile& projectile, const Atmosphere& atmosphere, const std::vector<VelocityObservation>& observations,
        BcFitKind fit_kind, const std::vector<double>& band_minimum_velocities_mps = {}, std::size_t maximum_iterations = 30);

    [[nodiscard]] BuckshotPatternResult analyze_buckshot_pattern(const BuckshotPatternInput& input);

    [[nodiscard]] TrajectoryUncertaintyResult propagate_trajectory_uncertainty(const Projectile& projectile, const Atmosphere& atmosphere, const ZeroedTrajectory& baseline, double maximum_distance_m,
        double zero_range_m, double sight_height_m, double muzzle_velocity_multiplier, const UncertaintyInputs& uncertainty, const std::vector<double>& output_distances_m,
        double output_interval_m = 0.25, const SolverConfiguration& configuration = SolverConfiguration {});

    [[nodiscard]] MonteCarloUncertaintyResult propagate_monte_carlo_uncertainty(const Projectile& projectile, const ShotScenario& baseline_scenario, const UncertaintyInputs& uncertainty,
        const std::vector<UncertaintyCorrelation>& correlations, const std::vector<double>& output_distances_m, std::size_t sample_count = 1000, std::uint64_t seed = 0x42574d43ULL);

    [[nodiscard]] SpinDriftResult compute_spin_drift(
        const Projectile& projectile, double time_of_flight_s, double twist_rate_inches, double muzzle_velocity_mps, const Atmosphere& atmosphere, int twist_direction_sign = 1);
