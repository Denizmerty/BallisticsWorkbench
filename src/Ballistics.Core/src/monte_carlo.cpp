...
namespace ballistics
{
    namespace
    {
        ...
        std::size_t variable_index(UncertaintyVariable variable)
        {
            switch (variable)
            {
        ...
        std::array<double, variable_count> standard_deviations(const UncertaintyInputs& inputs)
        {
            return { inputs.muzzle_velocity_standard_deviation_mps,
                inputs.drag_relative_standard_deviation,
                inputs.temperature_standard_deviation_c,
                inputs.pressure_standard_deviation_hpa,
                inputs.headwind_standard_deviation_mps,
                inputs.crosswind_standard_deviation_mps,
                inputs.zero_range_standard_deviation_m };
        }

        using Matrix = std::array<std::array<double, variable_count>, variable_count>;

        Matrix correlation_cholesky(const std::vector<UncertaintyCorrelation>& correlations)
        {
            Matrix correlation {};
            ...
            for (const auto& item : correlations)
            {
                ...
                if (first == second || !std::isfinite(item.coefficient) || std::abs(item.coefficient) >= 1.0)
                {
                    throw std::invalid_argument("invalid uncertainty correlation");
                }
                if (declared[first][second] != 0.0 && std::abs(correlation[first][second] - item.coefficient) > 1e-12)
                {
            ...
            for (std::size_t row = 0; row < variable_count; ++row)
            {
                for (std::size_t column = 0; column <= row; ++column)
                {
                    ...
                    if (row == column)
                    {
                        if (value < -1e-12)
                        {
                            throw std::invalid_argument("uncertainty correlation matrix is not positive semidefinite");
                        }
                    ...
                    else
                    {
                        if (lower[column][column] <= 1e-12)
                        {
                            ...
                            {
                                throw std::invalid_argument("uncertainty correlation matrix has a singular inconsistent row");
                            }
        ...
        std::array<double, variable_count> correlated_normal(NormalGenerator& generator, const Matrix& lower)
        {
        ...
        UncertaintyInterval interval(const std::vector<double>& values)
        {
            return { quantile(values, 0.5), quantile(values, 0.025), quantile(values, 0.975) };
        }

        double split_quantile_delta(const std::vector<double>& values)
        {
            if (values.size() < 40)
            {
        ...
        double sight_line_elevation(const ShotGeometry& geometry)
        {
            return std::abs(geometry.target_elevation_m) > 1e-12 ? std::atan2(geometry.target_elevation_m, geometry.target_distance_m) : geometry.target_inclination_rad;
        }
    ...
    MonteCarloUncertaintyResult propagate_monte_carlo_uncertainty(const Projectile& projectile, const ShotScenario& baseline_scenario, const UncertaintyInputs& uncertainty,
        const std::vector<UncertaintyCorrelation>& correlations, const std::vector<double>& output_distances_m, std::size_t sample_count, std::uint64_t seed)
    {
        if (sample_count < 100 || sample_count > 10000 || output_distances_m.empty() || !std::is_sorted(output_distances_m.begin(), output_distances_m.end()) || output_distances_m.front() < 0.0 ||
        ...
        if (std::any_of(deviations.begin(),
                deviations.end(),
                [](double value)
                {
        ...
        const auto active_inputs = std::count_if(deviations.begin(),
            deviations.end(),
            [](double value)
            {
                return value > 0.0;
        ...
        for (std::size_t sample_index = 0; sample_index < sample_count; ++sample_index)
        {
            ...
            const auto temperature_c = std::clamp(baseline_atmosphere.temperature_c + random[2] * deviations[2], -60.0, 60.0);
            const auto pressure_hpa = std::clamp(baseline_atmosphere.station_pressure_hpa + random[3] * deviations[3], 500.0, 1100.0);
            const auto headwind_mps = std::clamp(baseline_atmosphere.headwind_mps + random[4] * deviations[4], -100.0, 100.0);
            const auto crosswind_mps = std::clamp(baseline_atmosphere.crosswind_mps + random[5] * deviations[5], -100.0, 100.0);
            scenario.environment.firing_point = Atmosphere::create(temperature_c, pressure_hpa, baseline_atmosphere.relative_humidity_percent, headwind_mps, crosswind_mps);

            const auto baseline_velocity = projectile.ammunition.muzzle_velocity_mps * baseline_scenario.muzzle_velocity_multiplier;
            const auto sampled_velocity = std::clamp(baseline_velocity + random[0] * deviations[0], projectile.ammunition.muzzle_velocity_mps * 0.5, projectile.ammunition.muzzle_velocity_mps * 1.5);
            scenario.muzzle_velocity_multiplier = sampled_velocity / projectile.ammunition.muzzle_velocity_mps;
            scenario.geometry.zero_range_m = std::clamp(baseline_scenario.geometry.zero_range_m + random[6] * deviations[6], 5.0, 1000.0);
            const auto drag_log_delta = random[1] * deviations[1];
            scenario.solver.aerodynamic_drag_multiplier = baseline_scenario.solver.aerodynamic_drag_multiplier * std::exp(reference_bc ? -drag_log_delta : drag_log_delta);

            const auto zeroed = integrate_shot_scenario(projectile, scenario);
            if (zeroed.status != ZeroingStatus::complete || zeroed.trajectory.covered_distance_m + 1e-9 < output_distances_m.back())
            {
                continue;
            }
            ...
            for (std::size_t output_index = 0; output_index < samples.size(); ++output_index)
            {
                ...
                const auto path_m = line_of_sight_path_m(sample, scenario.geometry.sight_height_m, sight_elevation_rad);
                const std::array<double, metric_count> metrics {
                    sample.ground_speed_mps, sample.energy_j, sample.momentum_kgms, sample.time_s, sample.drop_m, path_m, elevation_holdover_rad(path_m, sample.distance_m), sample.wind_drift_m
                };
        ...
        for (std::size_t output_index = 0; output_index < result.samples.size(); ++output_index)
        {
            ...
            {
                result.maximum_split_quantile_delta = std::max(result.maximum_split_quantile_delta, split_quantile_delta(values_for_metric));
            }
        }
        result.status = active_inputs == 0 ? UncertaintyStatus::no_inputs : result.completed_sample_count == sample_count ? UncertaintyStatus::complete
