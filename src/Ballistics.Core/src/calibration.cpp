...
namespace ballistics
{
    namespace
    {
        ...
        bool solve_linear_system(Matrix matrix, std::vector<double> right_hand_side, std::vector<double>& solution)
        {
            const auto size = matrix.size();
            if (size == 0 || right_hand_side.size() != size ||
                std::any_of(matrix.begin(),
                    matrix.end(),
                    [size](const auto& row)
                    {
            ...
            return std::all_of(solution.begin(),
                solution.end(),
                [](double value)
                {
        ...
        bool invert_matrix(const Matrix& matrix, Matrix& inverse)
        {
            const auto size = matrix.size();
    ...
    BcCalibrationResult calibrate_reference_ballistic_coefficient(const Projectile& projectile, const Atmosphere& atmosphere, const std::vector<VelocityObservation>& observations, BcFitKind fit_kind,
        const std::vector<double>& band_minimum_velocities_mps, std::size_t maximum_iterations)
    {
        ...
        else
        {
            ...
            {
                throw std::invalid_argument("Velocity-banded calibration requires 2 to 4 thresholds beginning at 0 m/s.");
            }
            for (std::size_t index = 0; index < thresholds.size(); ++index)
            {
                if (!std::isfinite(thresholds[index]) || thresholds[index] < 0.0 || thresholds[index] > 2000.0 || (index > 0 && thresholds[index] <= thresholds[index - 1]))
                {
                    throw std::invalid_argument("Calibration band thresholds must be finite and strictly increasing.");
                }
            }
        }
        ...
        {
            if (!std::isfinite(observation.distance_m) || observation.distance_m <= 0.0 || observation.distance_m > 2000.0 || observation.distance_m <= previous_distance)
            {
                throw std::invalid_argument("Observation distances must be strictly increasing between 0 and 2000 m.");
            }
            if (!std::isfinite(observation.measured_velocity_mps) || observation.measured_velocity_mps < 1.0 || observation.measured_velocity_mps > 2000.0)
            {
                throw std::invalid_argument("Observed velocity must be between 1 and 2000 m/s.");
            }
            if (!std::isfinite(observation.standard_deviation_mps) || observation.standard_deviation_mps < 0.01 || observation.standard_deviation_mps > 200.0)
            {
                throw std::invalid_argument("Observation standard deviation must be between 0.01 and 200 m/s.");
            }
        ...
        const auto build_projectile = [&](const std::vector<double>& values)
        {
            ...
            BallisticCoefficientDefinition coefficient = ConstantBallisticCoefficient { std::exp(values.front()) };
            ...
            candidate.definition.drag = ReferenceBcDrag { reference_drag->curve, std::move(coefficient) };
            return candidate;
        };
        ...
        const auto predict = [&](const std::vector<double>& values, std::vector<double>& predictions)
        {
            try
            {
                ...
                const auto trajectory = integrate_trajectory(candidate, atmosphere, maximum_distance, 1.0, 0.25, SolverConfiguration {});
                predictions.clear();
        ...
        const auto evaluate = [&](const std::vector<double>& values, std::vector<double>& residuals, std::vector<double>* all_predictions = nullptr)
        {
            ...
            for (std::size_t index = 0; index < observations.size(); ++index)
            {
                ...
                const auto residual = (predictions[index] - observations[index].measured_velocity_mps) / observations[index].standard_deviation_mps;
        ...
        for (std::size_t iteration = 0; iteration < maximum_iterations; ++iteration)
        {
            ...
            const auto maximum_gradient = std::max_element(gradient.begin(),
                gradient.end(),
            ...
            for (int attempt = 0; attempt < 8; ++attempt)
            {
                ...
                const auto maximum_trial_delta = *std::max_element(delta.begin(),
                    delta.end(),
                    [](double first, double second)
                    {
                        return std::abs(first) < std::abs(second);
            ...
            const auto maximum_delta = *std::max_element(accepted_delta.begin(),
                accepted_delta.end(),
                [](double first, double second)
                {
                    return std::abs(first) < std::abs(second);
        ...
        result.status = converged ? CalibrationStatus::converged : CalibrationStatus::maximum_iterations;
        ...
        for (std::size_t index = 0; index < observations.size(); ++index)
        {
            ...
            result.residuals.push_back({ observations[index].distance_m,
        ...
        result.calibration_rmse_mps = std::sqrt(calibration_squared_error / static_cast<double>(calibration_count));
        result.weighted_rmse = std::sqrt(objective / static_cast<double>(calibration_count));
        if (holdout_count > 0)
        {
            result.holdout_rmse_mps = std::sqrt(holdout_squared_error / static_cast<double>(holdout_count));
        }
        const auto degrees_of_freedom = calibration_count - parameters.size();
        result.reduced_chi_square = degrees_of_freedom > 0 ? objective / static_cast<double>(degrees_of_freedom) : 0.0;
        ...
        if (final_derivative_valid)
        {
            for (std::size_t row = 0; row < calibration_count; ++row)
            {
                for (std::size_t first = 0; first < parameters.size(); ++first)
                {
                    for (std::size_t second = 0; second < parameters.size(); ++second)
                    {
                        final_normal[first][second] += final_jacobian[row][first] * final_jacobian[row][second];
                    }
                }
            }
        }
        Matrix covariance;
        const auto covariance_valid = degrees_of_freedom > 0 && final_derivative_valid && invert_matrix(final_normal, covariance);
        if (!covariance_valid)
        {
        ...
        for (std::size_t index = 0; index < parameters.size(); ++index)
        {
            ...
            if (covariance_valid && covariance[index][index] >= 0.0 && std::isfinite(covariance[index][index]))
            {
                const auto standard_error = std::sqrt(covariance[index][index] * variance_scale);
                estimate.confidence_95_low = std::clamp(std::exp(parameters[index] - 1.959963984540054 * standard_error), minimum_bc, maximum_bc);
                estimate.confidence_95_high = std::clamp(std::exp(parameters[index] + 1.959963984540054 * standard_error), minimum_bc, maximum_bc);
            }
