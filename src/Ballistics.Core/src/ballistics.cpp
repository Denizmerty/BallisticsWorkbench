...
namespace ballistics
{
    namespace
    {
        ...
        constexpr std::array<G1Band, 40> g1_table { { { 4230, 1.477404177730177e-4, 1.9565 },
            { 3680, 1.920339268755614e-4, 1.925 },
            { 3450, 2.894751026819746e-4, 1.875 },
            { 3295, 4.349905111115636e-4, 1.825 },
            { 3130, 6.520421871892662e-4, 1.775 },
            { 2960, 9.748073985e-4, 1.725 },
            { 2830, 1.453721560286e-3, 1.675 },
            { 2680, 2.162887202930376e-3, 1.625 },
            { 2460, 3.209559783129881e-3, 1.575 },
            { 2225, 3.904368218691249e-3, 1.55 },
            { 2015, 3.222942271262614e-3, 1.575 },
            { 1890, 2.203329542809e-3, 1.625 },
            { 1810, 1.511001028891904e-3, 1.675 },
            { 1730, 8.609957592468259e-4, 1.75 },
            { 1595, 4.086146797305117e-4, 1.85 },
            { 1520, 1.954473210037398e-4, 1.95 },
            { 1420, 5.431896266462351e-5, 2.125 },
            { 1360, 8.847742581674416e-6, 2.375 },
            { 1315, 1.456922328720298e-6, 2.625 },
            { 1280, 2.419485191895565e-7, 2.875 },
            { 1220, 1.657956321067612e-8, 3.25 },
            { 1185, 4.745469537371e-10, 3.75 },
            { 1150, 1.379746590025088e-11, 4.25 },
            { 1100, 4.070157961147882e-13, 4.75 },
            { 1060, 2.938236954847331e-14, 5.125 },
            { 1025, 1.228597370014e-14, 5.25 },
            { 980, 2.916938264100495e-14, 5.125 },
            { 945, 3.855099424807451e-13, 4.75 },
            { 905, 3.953721585280791e-12, 4.5 },
            { 860, 1.051728804013e-10, 4.0 },
            { 810, 1.485749033551948e-9, 3.75 },
            { 780, 1.041162981722437e-8, 3.5 },
            { 750, 1.231911859621178e-7, 3.0 },
            { 700, 4.600998799e-7, 2.875 },
            { 640, 3.209559783129881e-6, 2.5 },
            { 600, 7.566129470914212e-6, 2.375 },
            { 550, 1.490005193169e-5, 2.25 },
            { 250, 1.241662783e-4, 1.875 },
            { 0, 1.477404177730177e-4, 1.9565 },
            { -1, 0, 0 } } };
        ...
        double interpolate(double x, const std::vector<std::pair<double, double>>& table)
        {
            ...
            auto hi = std::upper_bound(table.begin(),
                table.end(),
                x,
                [](double value, const auto& item)
                {
        ...
        const std::vector<std::pair<double, double>>& g7_table()
        {
            ...
            static const std::vector<std::pair<double, double>> table { { 0.0, 0.1198 },
                { 0.05, 0.1197 },
                { 0.1, 0.1196 },
                { 0.15, 0.1194 },
                { 0.2, 0.1193 },
        ...
        double cubic_bezier_derivative(const std::array<double, 4>& v, double u)
        {
            const auto q = 1.0 - u;
        ...
        double bezier_y_from_x(double x, const std::array<std::pair<double, double>, 4>& points)
        {
            ...
            const auto x_tolerance = 8.0 * std::numeric_limits<double>::epsilon() * std::max({ 1.0, std::abs(x), std::abs(xs[0]), std::abs(xs[3]) });
            ...
            for (int iteration = 0; iteration < 12; ++iteration)
            {
                ...
                const auto candidate = std::abs(derivative) > std::numeric_limits<double>::epsilon() ? u - residual / derivative : std::numeric_limits<double>::quiet_NaN();
                u = std::isfinite(candidate) && candidate > low && candidate < high ? candidate : (low + high) / 2.0;
            }
        ...
        std::array<double, 3> sphere_aerodynamics(double speed, double diameter, const Atmosphere& atmosphere)
        {
            ...
            const auto reynolds = atmosphere.density_kg_m3 * speed * diameter / atmosphere.dynamic_viscosity_pa_s;
        ...
        double physical_drag_acceleration(double drag_coefficient, double reference_area_m2, double mass_kg, double speed_mps, const Atmosphere& atmosphere)
        {
            if (drag_coefficient < 0.0 || reference_area_m2 <= 0.0 || mass_kg <= 0.0)
            {
                throw std::invalid_argument("invalid physical drag inputs");
            }
            return 0.5 * atmosphere.density_kg_m3 * drag_coefficient * reference_area_m2 * speed_mps * speed_mps / mass_kg;
        }

        double reference_drag_acceleration(double drag_coefficient, double ballistic_coefficient, double speed_mps, const Atmosphere& atmosphere)
        {
            const auto reference_area_m2 = pi * inches_to_m * inches_to_m / 4.0;
            const auto reference_mass_kg = ballistic_coefficient * lb_to_kg;
            return physical_drag_acceleration(drag_coefficient, reference_area_m2, reference_mass_kg, speed_mps, atmosphere);
        }
        ...
        LegacyState legacy_derivative(const LegacyState& s, const Projectile& p, const Atmosphere& a)
        {
        ...
        LegacyState legacy_rk4(const LegacyState& s, double dx, const Projectile& p, const Atmosphere& a)
        {
        ...
        TimeState time_derivative(const TimeState& s, const Projectile& p, const Environment& environment, bool include_drag, bool include_gravity, double aerodynamic_drag_multiplier)
        {
            ...
            const auto relative = std::hypot(rx, ry, rz);
            const auto retard = include_drag ? aerodynamic_drag_multiplier * drag_retardation_mps2(relative, p, atmosphere) : 0.0;
            ...
            return { s[3], s[4], s[5], drag_scale * rx + coriolis.x, drag_scale * ry - gravity + coriolis.y, drag_scale * rz + coriolis.z };
        }
        ...
        DormandPrinceStep dormand_prince_54(
            const TimeState& s, double dt, const Projectile& p, const Environment& environment, bool include_drag, bool include_gravity, double aerodynamic_drag_multiplier)
        {
            const auto k1 = time_derivative(s, p, environment, include_drag, include_gravity, aerodynamic_drag_multiplier);
            TimeState stage {};
            const auto evaluate = [&](const std::initializer_list<std::pair<double, const TimeState*>>& terms)
            {
                ...
                return time_derivative(stage, p, environment, include_drag, include_gravity, aerodynamic_drag_multiplier);
            };
            ...
            const auto k5 = evaluate({ { 19372.0 / 6561.0, &k1 }, { -25360.0 / 2187.0, &k2 }, { 64448.0 / 6561.0, &k3 }, { -212.0 / 729.0, &k4 } });
            const auto k6 = evaluate({ { 9017.0 / 3168.0, &k1 }, { -355.0 / 33.0, &k2 }, { 46732.0 / 5247.0, &k3 }, { 49.0 / 176.0, &k4 }, { -5103.0 / 18656.0, &k5 } });
            const auto k7 = evaluate({ { 35.0 / 384.0, &k1 }, { 500.0 / 1113.0, &k3 }, { 125.0 / 192.0, &k4 }, { -2187.0 / 6784.0, &k5 }, { 11.0 / 84.0, &k6 } });

            DormandPrinceStep result {};
            for (std::size_t component = 0; component < s.size(); ++component)
            {
                result.fifth_order[component] =
                    s[component] + dt * (35.0 / 384.0 * k1[component] + 500.0 / 1113.0 * k3[component] + 125.0 / 192.0 * k4[component] - 2187.0 / 6784.0 * k5[component] + 11.0 / 84.0 * k6[component]);
                result.fourth_order[component] = s[component] +
                    dt *
                        (5179.0 / 57600.0 * k1[component] + 7571.0 / 16695.0 * k3[component] + 393.0 / 640.0 * k4[component] - 92097.0 / 339200.0 * k5[component] + 187.0 / 2100.0 * k6[component] +
                            1.0 / 40.0 * k7[component]);
            }
            return result;
        }

        double error_norm(const TimeState& before, const DormandPrinceStep& step, const SolverConfiguration& configuration)
        {
            double result = 0.0;
            for (std::size_t component = 0; component < before.size(); ++component)
            {
                const auto absolute_tolerance = component < 3 ? configuration.absolute_position_tolerance_m : configuration.absolute_velocity_tolerance_mps;
                const auto scale = absolute_tolerance + configuration.relative_tolerance * std::max(std::abs(before[component]), std::abs(step.fifth_order[component]));
                result = std::max(result, std::abs(step.fifth_order[component] - step.fourth_order[component]) / scale);
            }
            return result;
        }

        double hermite_position(double p0, double v0, double p1, double v1, double dt, double u)
        {
            const auto u2 = u * u;
            const auto u3 = u2 * u;
            return (2.0 * u3 - 3.0 * u2 + 1.0) * p0 + (u3 - 2.0 * u2 + u) * dt * v0 + (-2.0 * u3 + 3.0 * u2) * p1 + (u3 - u2) * dt * v1;
        }

        double hermite_velocity(double p0, double v0, double p1, double v1, double dt, double u)
        {
            const auto u2 = u * u;
            return ((6.0 * u2 - 6.0 * u) * p0 + (3.0 * u2 - 4.0 * u + 1.0) * dt * v0 + (-6.0 * u2 + 6.0 * u) * p1 + (3.0 * u2 - 2.0 * u) * dt * v1) / dt;
        }

        TimeState interpolate_time_state(const TimeState& before, const TimeState& after, double dt, double u)
        {
            TimeState result {};
            for (std::size_t axis = 0; axis < 3; ++axis)
            {
                result[axis] = hermite_position(before[axis], before[axis + 3], after[axis], after[axis + 3], dt, u);
                result[axis + 3] = hermite_velocity(before[axis], before[axis + 3], after[axis], after[axis + 3], dt, u);
            }
            return result;
        }

        double interpolation_fraction_for_x(const TimeState& before, const TimeState& after, double dt, double target_x)
        {
            double low = 0.0;
            double high = 1.0;
        ...
        bool finite_state(const TimeState& state)
        {
            return std::all_of(state.begin(),
                state.end(),
                [](double value)
                {
        ...
        void validate_solver_configuration(const SolverConfiguration& configuration)
        {
            const auto positive_finite = [](double value)
            {
                return std::isfinite(value) && value > 0.0;
            };
            if (!positive_finite(configuration.relative_tolerance) || !positive_finite(configuration.absolute_position_tolerance_m) ||
                !positive_finite(configuration.absolute_velocity_tolerance_mps) || !positive_finite(configuration.initial_time_step_s) || !positive_finite(configuration.minimum_time_step_s) ||
                !positive_finite(configuration.maximum_time_step_s) || !positive_finite(configuration.maximum_time_s) || !std::isfinite(configuration.minimum_forward_speed_mps) ||
                configuration.minimum_forward_speed_mps < 0.0 || !std::isfinite(configuration.launch_elevation_rad) || std::abs(configuration.launch_elevation_rad) >= pi / 2.0 ||
                !std::isfinite(configuration.launch_azimuth_rad) || std::abs(configuration.launch_azimuth_rad) >= pi / 2.0 || !std::isfinite(configuration.sight_line_elevation_rad) ||
                std::abs(configuration.sight_line_elevation_rad) >= pi / 2.0 || !std::isfinite(configuration.ground_height_m) || configuration.maximum_steps == 0 ||
                !positive_finite(configuration.aerodynamic_drag_multiplier) || configuration.minimum_time_step_s > configuration.maximum_time_step_s)
            {
    ...
    DragModel drag_model(const Projectile& projectile)
    {
        ...
        return std::holds_alternative<TabulatedDrag>(projectile.definition.drag) ? DragModel::tabulated_cd : DragModel::sphere;
    }

    const ReferenceBcDrag* reference_bc_drag(const Projectile& projectile)
    {
        return std::get_if<ReferenceBcDrag>(&projectile.definition.drag);
    }

    ReferenceBcDrag* reference_bc_drag(Projectile& projectile)
    {
        return std::get_if<ReferenceBcDrag>(&projectile.definition.drag);
    }

    const TabulatedDrag* tabulated_drag(const Projectile& projectile)
    {
        return std::get_if<TabulatedDrag>(&projectile.definition.drag);
    }

    TabulatedDrag* tabulated_drag(Projectile& projectile)
    {
        return std::get_if<TabulatedDrag>(&projectile.definition.drag);
    }

    const SphereDrag* sphere_drag(const Projectile& projectile)
    {
        return std::get_if<SphereDrag>(&projectile.definition.drag);
    }

    SphereDrag* sphere_drag(Projectile& projectile)
    {
        return std::get_if<SphereDrag>(&projectile.definition.drag);
    }

    double nominal_ballistic_coefficient(const Projectile& projectile)
    {
    ...
    double elevation_holdover_rad(double path_m, double distance_m)
    {
    ...
    double sphere_drag_vs_reynolds(double re)
    {
        ...
        return 24.0 / re + (2.6 * (re / 5.0)) / (1.0 + std::pow(re / 5.0, 1.52)) + (0.411 * std::pow(re / 263000.0, -7.94) / (1.0 + std::pow(re / 263000.0, -8.0))) + std::pow(re, 0.8) / 461000.0;
    }

    double sphere_drag_coefficient(double mach, double reynolds)
    {
        ...
        const auto shock = corrected >= sphere_supported_mach_max ? 1.0 : bezier_y_from_x(corrected, shock_points);
    ...
    double g7_drag_coefficient(double mach)
    {
        if (!std::isfinite(mach) || mach < 0.0)
        {
            throw std::invalid_argument("Mach number must be finite and non-negative");
    ...
    double g1_drag_coefficient(double mach)
    {
        if (!std::isfinite(mach) || mach < 0.0)
        {
            throw std::invalid_argument("Mach number must be finite and non-negative");
        ...
        const auto standard_retardation_mps2 = g1_retardation(standard_speed_mps * mps_to_fps, 1.0) * fps_to_mps;
        return standard_retardation_mps2 / (k_cd_ref * standard_speed_mps * standard_speed_mps);
    }

    double tabulated_drag_coefficient(const Projectile& p, double mach)
    {
        if (!std::isfinite(mach) || mach < 0.0)
        {
            throw std::invalid_argument("Mach number must be finite and non-negative");
        ...
        const auto high = std::upper_bound(drag->points.begin(),
            drag->points.end(),
            mach,
            [](double value, const MachCdPoint& point)
            {
    ...
    double effective_ballistic_coefficient(const Projectile& p, double airspeed)
    {
        ...
        const auto selected = std::upper_bound(bands.begin(),
            bands.end(),
            airspeed,
    ...
    double drag_retardation_mps2(double speed, const Projectile& p, const Atmosphere& a)
    {
        ...
        {
            const auto area = pi * tabulated->reference_diameter_m * tabulated->reference_diameter_m / 4.0;
            return physical_drag_acceleration(tabulated_drag_coefficient(p, mach), area, p.definition.mass_kg, speed, a);
        }
        ...
        const auto reference_cd = reference->curve == ReferenceDragCurve::g7 ? g7_drag_coefficient(mach) : g1_drag_coefficient(mach);
        return reference_drag_acceleration(reference_cd, effective_ballistic_coefficient(p, speed), speed, a);
    }

    AerodynamicDiagnostics aerodynamic_diagnostics(const Projectile& p, const Vec3& ground_velocity, const Atmosphere& a)
    {
        const Vec3 air_relative { ground_velocity.x + a.headwind_mps, ground_velocity.y, ground_velocity.z - a.crosswind_mps };
        ...
        if (const auto* tabulated = tabulated_drag(p))
        {
            const auto reynolds = a.density_kg_m3 * airspeed * tabulated->reference_diameter_m / a.dynamic_viscosity_pa_s;
            return { ground_speed, airspeed, tabulated_drag_coefficient(p, mach), reynolds, mach, true, true };
        }
    ...
    std::optional<TrajectorySample> Trajectory::sample_at(double distance) const
    {
        constexpr double tolerance = 1e-9;
        if (samples.empty() || !std::isfinite(distance) || distance < samples.front().distance_m - tolerance || distance > samples.back().distance_m + tolerance)
        {
            return std::nullopt;
        }

        const auto x = std::clamp(distance, samples.front().distance_m, samples.back().distance_m);
        const auto hi = std::lower_bound(samples.begin(),
            samples.end(),
            x,
        ...
        result.air_relative_velocity_mps = vector_lerp(a.air_relative_velocity_mps, b.air_relative_velocity_mps);
    ...
    Trajectory integrate_trajectory_legacy(const Projectile& p, const Atmosphere& a, double max_distance, double multiplier, double dx)
    {
        ...
        const auto append_sample = [&](double downrange, const LegacyState& current)
        {
            ...
            const Vec3 air_relative { current[3] + a.headwind_mps, current[4], current[5] - a.crosswind_mps };
            const auto diagnostics = aerodynamic_diagnostics(p, ground_velocity, a);
            trajectory.samples.push_back({ downrange,
                { downrange, current[1], current[2] },
                ground_velocity,
                air_relative,
                diagnostics.ground_speed_mps,
                diagnostics.airspeed_mps,
                0.5 * p.definition.mass_kg * diagnostics.ground_speed_mps * diagnostics.ground_speed_mps,
    ...
    Trajectory integrate_trajectory(const Projectile& p, const Atmosphere& a, double max_distance, double multiplier, double output_interval)
    {
        return integrate_trajectory(p, a, max_distance, multiplier, output_interval, SolverConfiguration {});
    }

    Trajectory integrate_trajectory(const Projectile& p, const Atmosphere& a, double max_distance, double multiplier, double output_interval, const SolverConfiguration& configuration)
    {
        ...
        return integrate_trajectory(p, Environment::homogeneous(a), max_distance, multiplier, output_interval, configuration);
    }

    Trajectory integrate_trajectory(const Projectile& p, const Environment& environment, double max_distance, double multiplier, double output_interval, const SolverConfiguration& configuration)
    {
        if (configuration.mode == SolverMode::legacy_distance)
        {
        ...
        TimeState state { 0.0, 0.0, 0.0, v0 * cos_elevation * cos_azimuth, v0 * std::sin(configuration.launch_elevation_rad), v0 * cos_elevation * std::sin(configuration.launch_azimuth_rad) };
        double time = 0.0;
        double dt = std::clamp(configuration.initial_time_step_s, configuration.minimum_time_step_s, configuration.maximum_time_step_s);
        ...
        const auto append_sample = [&](double downrange, const TimeState& current, double sample_time)
        {
            ...
            const auto atmosphere = environment.atmosphere_at(position);
            const Vec3 ground_velocity { current[3], current[4], current[5] };
            const Vec3 air_relative { current[3] + atmosphere.headwind_mps, current[4], current[5] - atmosphere.crosswind_mps };
        ...
        while (!finished)
        {
            ...
            const auto step = dormand_prince_54(before, dt, p, environment, configuration.include_aerodynamic_drag, configuration.include_gravity, configuration.aerodynamic_drag_multiplier);
            ++trajectory.solver.attempted_steps;
            ...
            trajectory.solver.maximum_error_norm = std::max(trajectory.solver.maximum_error_norm, normalized_error);
            ...
            {
                trajectory.solver.minimum_accepted_time_step_s = std::min(trajectory.solver.minimum_accepted_time_step_s, dt);
                trajectory.solver.maximum_accepted_time_step_s = std::max(trajectory.solver.maximum_accepted_time_step_s, dt);
            }
            trajectory.solver.final_time_step_s = dt;

            auto append_regular_samples = [&](double limit_distance)
            {
                while (next_sample_distance <= limit_distance + 1e-12 && next_sample_distance <= max_distance + 1e-12)
                {
                    ...
                    append_sample(target, interpolate_time_state(before, state, dt, u), before_time + u * dt);
            ...
            if (state[0] >= max_distance)
            {
                ...
                if (trajectory.samples.back().distance_m < max_distance - 1e-10)
                {
                    ...
                    append_sample(max_distance, interpolate_time_state(before, state, dt, u), before_time + u * dt);
                }
            ...
            else if (configuration.terminate_at_ground && before[1] >= configuration.ground_height_m && state[1] < configuration.ground_height_m)
            {
                double low = 0.0;
                double high = 1.0;
                for (int iteration = 0; iteration < 60; ++iteration)
            ...
            if (!finished)
            {
                const auto factor = normalized_error == 0.0 ? 5.0 : std::clamp(0.9 * std::pow(normalized_error, -0.2), 0.2, 5.0);
                dt = std::clamp(dt * factor, configuration.minimum_time_step_s, configuration.maximum_time_step_s);
            }
        }
