...
namespace ballistics
{
    ...
    TrajectoryUncertaintyResult propagate_trajectory_uncertainty(const Projectile& projectile, const Atmosphere& atmosphere, const ZeroedTrajectory& baseline, double maximum_distance_m,
        double zero_range_m, double sight_height_m, double muzzle_velocity_multiplier, const UncertaintyInputs& uncertainty, const std::vector<double>& output_distances_m, double output_interval_m,
        const SolverConfiguration& configuration)
    {
        ...
        for (const auto& [value, maximum] : bounded_inputs)
        {
            ...
            {
                throw std::invalid_argument("Uncertainty inputs must be finite, non-negative, and bounded.");
            }
        }
        if (!std::isfinite(maximum_distance_m) || maximum_distance_m < 0.0 || !std::isfinite(zero_range_m) || zero_range_m < 5.0 || zero_range_m > 1000.0 || !std::isfinite(sight_height_m) ||
            sight_height_m < 0.0 || sight_height_m > 0.25 || !std::isfinite(muzzle_velocity_multiplier) || muzzle_velocity_multiplier < 0.5 || muzzle_velocity_multiplier > 1.5)
        {
            throw std::invalid_argument("Uncertainty propagation geometry is invalid.");
        }
        if (output_distances_m.empty())
        {
            throw std::invalid_argument("Uncertainty propagation requires at least one output distance.");
        }
        double previous_distance = -1.0;
        for (const auto distance : output_distances_m)
        {
            if (!std::isfinite(distance) || distance < 0.0 || distance > maximum_distance_m || distance <= previous_distance)
            {
                throw std::invalid_argument("Uncertainty output distances must be strictly increasing within the trajectory.");
            }
        ...
        result.active_input_count = static_cast<std::size_t>(std::count_if(bounded_inputs.begin(),
            bounded_inputs.end(),
            [](const auto& item)
            {
                return item.first > 0.0;
            }));
        ...
        const auto accumulate_pair = [&](const Trajectory& higher, const Trajectory& lower, double coordinate_span, double standard_deviation)
        {
            ...
            for (std::size_t index = 0; index < output_distances_m.size(); ++index)
            {
                ...
                output.speed_standard_deviation_mps += contribution(high->ground_speed_mps, low->ground_speed_mps);
                output.energy_standard_deviation_j += contribution(high->energy_j, low->energy_j);
                output.momentum_standard_deviation_kgms += contribution(high->momentum_kgms, low->momentum_kgms);
                ...
                output.path_standard_deviation_m += contribution(high->position_m.y - sight_height_m, low->position_m.y - sight_height_m);
                output.holdover_standard_deviation_rad +=
                    contribution(elevation_holdover_rad(high->position_m.y - sight_height_m, output.distance_m), elevation_holdover_rad(low->position_m.y - sight_height_m, output.distance_m));
                output.wind_drift_standard_deviation_m += contribution(high->wind_drift_m, low->wind_drift_m);
            }
            if (pair_complete)
            {
        ...
        const auto fixed_bore_trajectory = [&](const Atmosphere& perturbed_atmosphere, double perturbed_multiplier, double drag_multiplier = 1.0)
        {
            ...
            perturbed_configuration.aerodynamic_drag_multiplier = configuration.aerodynamic_drag_multiplier * drag_multiplier;
            return integrate_trajectory(projectile, perturbed_atmosphere, maximum_distance_m, perturbed_multiplier, output_interval_m, perturbed_configuration);
        };
        ...
        if (uncertainty.muzzle_velocity_standard_deviation_mps > 0.0)
        {
            ...
            const auto baseline_velocity = projectile.ammunition.muzzle_velocity_mps * muzzle_velocity_multiplier;
            const auto step = std::clamp(standard_deviation * 0.1, 0.05, 1.0);
            const auto lower_velocity = std::max(projectile.ammunition.muzzle_velocity_mps * 0.5, baseline_velocity - step);
            const auto higher_velocity = std::min(projectile.ammunition.muzzle_velocity_mps * 1.5, baseline_velocity + step);
            accumulate_pair(fixed_bore_trajectory(atmosphere, higher_velocity / projectile.ammunition.muzzle_velocity_mps),
                fixed_bore_trajectory(atmosphere, lower_velocity / projectile.ammunition.muzzle_velocity_mps),
        ...
        if (uncertainty.drag_relative_standard_deviation > 0.0)
        {
            ...
            accumulate_pair(fixed_bore_trajectory(atmosphere, muzzle_velocity_multiplier, higher_drag_multiplier),
                fixed_bore_trajectory(atmosphere, muzzle_velocity_multiplier, lower_drag_multiplier),
                2.0 * step,
                standard_deviation);
        }

        const auto atmosphere_pair = [&](double standard_deviation, double baseline_value, double minimum, double maximum, double minimum_step, double maximum_step, const auto& create_atmosphere)
        {
            ...
            accumulate_pair(fixed_bore_trajectory(create_atmosphere(higher_value), muzzle_velocity_multiplier),
                fixed_bore_trajectory(create_atmosphere(lower_value), muzzle_velocity_multiplier),
                higher_value - lower_value,
                standard_deviation);
        };

        atmosphere_pair(uncertainty.temperature_standard_deviation_c,
            ...
            [&](double value)
            {
                return Atmosphere::create(value, atmosphere.station_pressure_hpa, atmosphere.relative_humidity_percent, atmosphere.headwind_mps, atmosphere.crosswind_mps);
            });
        atmosphere_pair(uncertainty.pressure_standard_deviation_hpa,
            ...
            [&](double value)
            {
                return Atmosphere::create(atmosphere.temperature_c, value, atmosphere.relative_humidity_percent, atmosphere.headwind_mps, atmosphere.crosswind_mps);
            });
        atmosphere_pair(uncertainty.headwind_standard_deviation_mps,
            ...
            [&](double value)
            {
                return Atmosphere::create(atmosphere.temperature_c, atmosphere.station_pressure_hpa, atmosphere.relative_humidity_percent, value, atmosphere.crosswind_mps);
            });
        atmosphere_pair(uncertainty.crosswind_standard_deviation_mps,
            ...
            [&](double value)
            {
                return Atmosphere::create(atmosphere.temperature_c, atmosphere.station_pressure_hpa, atmosphere.relative_humidity_percent, atmosphere.headwind_mps, value);
            });
        ...
        if (uncertainty.zero_range_standard_deviation_m > 0.0)
        {
            ...
            const auto higher = integrate_zeroed_trajectory(projectile, atmosphere, maximum_distance_m, higher_range, sight_height_m, muzzle_velocity_multiplier, output_interval_m, configuration);
            const auto lower = integrate_zeroed_trajectory(projectile, atmosphere, maximum_distance_m, lower_range, sight_height_m, muzzle_velocity_multiplier, output_interval_m, configuration);
            if (higher.status == ZeroingStatus::complete && lower.status == ZeroingStatus::complete)
            {
                accumulate_pair(higher.trajectory, lower.trajectory, higher_range - lower_range, standard_deviation);
            }
            else
            {
                for (auto& sample : result.samples)
        ...
        for (auto& sample : result.samples)
        {
            ...
            sample.momentum_standard_deviation_kgms = std::sqrt(sample.momentum_standard_deviation_kgms);
        ...
        const auto every_sample_available = std::all_of(result.samples.begin(),
            result.samples.end(),
            [](const TrajectoryUncertaintySample& sample)
            {
                return sample.available;
            });
        result.status = result.completed_input_count == result.active_input_count && every_sample_available ? UncertaintyStatus::complete : UncertaintyStatus::partial;
        return result;
    }

    SpinDriftResult compute_spin_drift(const Projectile& p, double tof, double twist, double muzzle, const Atmosphere& a, int sign)
    {
        ...
        const auto velocity = std::pow(std::max(muzzle * metres_per_second_to_feet_per_second / 2800.0, 0.05), 1.0 / 3.0);
