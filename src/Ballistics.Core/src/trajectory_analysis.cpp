...
namespace ballistics
{
    ...
    ZeroedTrajectory integrate_zeroed_trajectory(
        const Projectile& p, const Atmosphere& a, double max_distance, double zero_range, double sight_height, double multiplier, double output_interval, const SolverConfiguration& configuration)
    {
        if (!std::isfinite(zero_range) || !std::isfinite(sight_height) || zero_range <= 0.0 || sight_height < 0.0 || configuration.mode != SolverMode::adaptive_time)
        {
        ...
        const auto evaluate = [&](double elevation) -> std::optional<double>
        {
            ...
            const auto candidate = integrate_trajectory(p, a, zero_range, multiplier, output_interval, solve_configuration);
            ...
            return line_of_sight_path_m(*endpoint, sight_height, configuration.sight_line_elevation_rad);
        };
        ...
        if (!low_error || !high_error)
        {
            ...
            return { integrate_trajectory(p, a, max_distance, multiplier, output_interval, solve_configuration), 0.0, 0.0, ZeroingStatus::range_unavailable };
        }
        ...
        {
            solve_configuration.launch_elevation_rad = configuration.sight_line_elevation_rad;
            return { integrate_trajectory(p, a, max_distance, multiplier, output_interval, solve_configuration), 0.0, 0.0, ZeroingStatus::no_solution };
        }
        ...
        auto trajectory = integrate_trajectory(p, a, max_distance, multiplier, output_interval, solve_configuration);
        if (max_distance >= zero_range)
        {
            if (const auto zero_sample = trajectory.sample_at(zero_range))
            {
                midpoint_error = line_of_sight_path_m(*zero_sample, sight_height, configuration.sight_line_elevation_rad);
            }
        }
        return { std::move(trajectory), midpoint, midpoint_error, ZeroingStatus::complete };
    }

    ZeroedTrajectory integrate_shot_scenario(const Projectile& projectile, const ShotScenario& scenario)
    {
        const auto& geometry = scenario.geometry;
        if (!std::isfinite(geometry.maximum_distance_m) || geometry.maximum_distance_m < 0.0 || !std::isfinite(geometry.zero_range_m) || geometry.zero_range_m <= 0.0 ||
            !std::isfinite(geometry.sight_height_m) || geometry.sight_height_m < 0.0 || !std::isfinite(geometry.target_distance_m) || geometry.target_distance_m < 0.0 ||
            !std::isfinite(geometry.target_elevation_m) || !std::isfinite(geometry.target_inclination_rad) || std::abs(geometry.target_inclination_rad) >= std::acos(-1.0) / 2.0 ||
            !std::isfinite(geometry.output_interval_m) || geometry.output_interval_m <= 0.0)
        {
            return { {}, 0.0, 0.0, ZeroingStatus::invalid_geometry };
        }
        if (std::abs(geometry.target_elevation_m) > 1e-12 && std::abs(geometry.target_inclination_rad) > 1e-12)
        {
            return { {}, 0.0, 0.0, ZeroingStatus::invalid_geometry };
        }

        auto configuration = scenario.solver;
        configuration.sight_line_elevation_rad = std::abs(geometry.target_elevation_m) > 1e-12 ? std::atan2(geometry.target_elevation_m, geometry.target_distance_m) : geometry.target_inclination_rad;
        ...
        const auto evaluate = [&](double elevation) -> std::optional<double>
        {
            ...
            const auto candidate =
                integrate_trajectory(projectile, scenario.environment, geometry.zero_range_m, scenario.muzzle_velocity_multiplier, geometry.output_interval_m, candidate_configuration);
            const auto endpoint = candidate.sample_at(geometry.zero_range_m);
            if (!endpoint || candidate.termination != TrajectoryTermination::requested_distance)
            {
                return std::nullopt;
            }
            return line_of_sight_path_m(*endpoint, geometry.sight_height_m, configuration.sight_line_elevation_rad);
        };
        ...
        auto low_error = evaluate(low);
        auto high_error = evaluate(high);
        for (int expansion = 0; expansion < 4 && low_error && high_error && (*low_error > 0.0 || *high_error < 0.0); ++expansion)
        {
        ...
        auto trajectory = integrate_trajectory(projectile, scenario.environment, geometry.maximum_distance_m, scenario.muzzle_velocity_multiplier, geometry.output_interval_m, configuration);
        if (const auto zero_sample = trajectory.sample_at(geometry.zero_range_m))
        {
            midpoint_error = line_of_sight_path_m(*zero_sample, geometry.sight_height_m, configuration.sight_line_elevation_rad);
        }
        return { std::move(trajectory), midpoint, midpoint_error, ZeroingStatus::complete };
    }

    TrajectoryEvents analyze_trajectory_events(const ZeroedTrajectory& zeroed, double sight_height, double sight_line_elevation_rad)
    {
        if (!std::isfinite(sight_height) || sight_height < 0.0 || !std::isfinite(sight_line_elevation_rad) || std::abs(sight_line_elevation_rad) >= std::acos(-1.0) / 2.0)
        {
            throw std::invalid_argument("Trajectory-event sight height must be finite and non-negative.");
        }
        ...
        {
            return std::abs(sample.ground_velocity_mps.x) > 1e-12 ? sample.ground_velocity_mps.y / sample.ground_velocity_mps.x : 0.0;
        };
        const auto range_value = [&](const TrajectorySample& first, const TrajectorySample& second, double fraction, double intercept, double slope)
        {
            ...
            return (2.0 * u3 - 3.0 * u2 + 1.0) * first_value + (u3 - 2.0 * u2 + fraction) * span * (range_slope(first) - slope) + (-2.0 * u3 + 3.0 * u2) * second_value +
                (u3 - u2) * span * (range_slope(second) - slope);
        };
        const auto range_derivative = [&](const TrajectorySample& first, const TrajectorySample& second, double fraction, double intercept, double slope)
        {
            const auto span = second.distance_m - first.distance_m;
            const auto u2 = fraction * fraction;
            const auto first_value = first.position_m.y - intercept - slope * first.distance_m;
            const auto second_value = second.position_m.y - intercept - slope * second.distance_m;
            return (6.0 * u2 - 6.0 * fraction) * first_value + (3.0 * u2 - 4.0 * fraction + 1.0) * span * (range_slope(first) - slope) + (-6.0 * u2 + 6.0 * fraction) * second_value +
                (3.0 * u2 - 2.0 * fraction) * span * (range_slope(second) - slope);
        };
        const auto root_fraction = [&](const TrajectorySample& first, const TrajectorySample& second, double intercept, double slope)
        {
            double low = 0.0;
            double high = 1.0;
        ...
        const auto range_at_fraction = [](const TrajectorySample& first, const TrajectorySample& second, double fraction)
        {
        ...
        for (std::size_t index = 1; index < trajectory.samples.size(); ++index)
        {
            ...
            const auto first_path = first.position_m.y - sight_height - sight_line_slope * first.distance_m;
            const auto second_path = second.position_m.y - sight_height - sight_line_slope * second.distance_m;

            if (!result.near_zero_m && first_path < -value_tolerance && second_path >= -value_tolerance)
            {
                result.near_zero_m = range_at_fraction(first, second, root_fraction(first, second, sight_height, sight_line_slope));
            }
            if (!result.far_zero_m && first_path > value_tolerance && second_path <= value_tolerance)
            {
                result.far_zero_m = range_at_fraction(first, second, root_fraction(first, second, sight_height, sight_line_slope));
            }

            if (!found_apex && range_slope(first) > sight_line_slope && range_slope(second) <= sight_line_slope)
            {
                double low = 0.0;
                double high = 1.0;
                for (int iteration = 0; iteration < 60; ++iteration)
                ...
                result.maximum_ordinate_path_m = range_value(first, second, fraction, sight_height, sight_line_slope);
                result.maximum_ordinate_status = TrajectoryEventStatus::complete;
                found_apex = true;
            }

            if (!found_ground && first.position_m.y > value_tolerance && second.position_m.y <= value_tolerance)
            {
                result.ground_intersection_m = range_at_fraction(first, second, root_fraction(first, second, 0.0, 0.0));
            ...
            for (const auto threshold : transonic_thresholds)
            {
                ...
                result.mach_crossings.push_back(
                    { threshold, range_at_fraction(first, second, std::clamp(fraction, 0.0, 1.0)), decelerating ? MachCrossingDirection::decelerating : MachCrossingDirection::accelerating });
            }
        }

        std::sort(result.mach_crossings.begin(),
        ...
        result.zero_crossings_status = result.near_zero_m && result.far_zero_m ? TrajectoryEventStatus::complete : TrajectoryEventStatus::horizon_limited;
        if (!found_apex)
        {
        ...
        {
            const auto crossing = std::find_if(result.mach_crossings.begin(),
                result.mach_crossings.end(),
    ...
    MpbrResult compute_mpbr(const Trajectory& t, double vital, double sight)
    {
        const auto radius = vital / 2.0;

        if (!std::isfinite(radius) || !std::isfinite(sight) || radius <= 0.0 || sight < 0.0 || sight > radius || t.samples.empty() || t.covered_distance_m < 5.0)
        {
        ...
        for (int i = 1; i <= bracket_steps; ++i)
        {
            ...
            if (std::isfinite(previous_value) && std::isfinite(value) && previous_value <= 0.0 && value >= 0.0)
            {
                lo = previous_zero;
                hi = candidate;
            }
        ...
        for (std::size_t i = 1; i < t.samples.size(); ++i)
        {
            ...
            if (t.samples[i].distance_m > zero && current < -radius)
            {
                if (previous != current)
                {
                    mpbr = t.samples[i - 1].distance_m + (previous + radius) / (previous - current) * (t.samples[i].distance_m - t.samples[i - 1].distance_m);
                }
                else
                {
    ...
    MpbrResult compute_native_mpbr(
        const Projectile& p, const Atmosphere& a, double max_distance, double vital, double sight, double multiplier, double output_interval, const SolverConfiguration& configuration)
    {
        const auto radius = vital / 2.0;
        if (!std::isfinite(radius) || !std::isfinite(sight) || !std::isfinite(max_distance) || radius <= 0.0 || sight < 0.0 || sight > radius || max_distance < 5.0 ||
        ...
        {
            solve_configuration.launch_elevation_rad = elevation;
            const auto trajectory = integrate_trajectory(p, a, max_distance, multiplier, output_interval, solve_configuration);
    ...
    MpbrResult compute_scenario_mpbr(const Projectile& projectile, const ShotScenario& scenario)
    {
        const auto& geometry = scenario.geometry;
        const auto radius = geometry.vital_zone_m / 2.0;
        if (!std::isfinite(radius) || radius <= 0.0 || !std::isfinite(geometry.sight_height_m) || geometry.sight_height_m < 0.0 || geometry.sight_height_m > radius ||
            !std::isfinite(geometry.maximum_distance_m) || geometry.maximum_distance_m < 5.0 || !std::isfinite(geometry.target_inclination_rad) || !std::isfinite(geometry.target_distance_m) ||
            geometry.target_distance_m < 0.0 || !std::isfinite(geometry.target_elevation_m) || (std::abs(geometry.target_inclination_rad) > 1e-12 && std::abs(geometry.target_elevation_m) > 1e-12))
        {
            return { 0.0, 0.0, MpbrStatus::invalid_geometry };
        }

        const auto sight_line_elevation_rad = std::abs(geometry.target_elevation_m) > 1e-12 ? std::atan2(geometry.target_elevation_m, geometry.target_distance_m) : geometry.target_inclination_rad;
        auto configuration = scenario.solver;
        ...
        const auto maximum_path = [&](double bore_elevation_rad) -> std::optional<double>
        {
            ...
            const auto trajectory =
                integrate_trajectory(projectile, scenario.environment, geometry.maximum_distance_m, scenario.muzzle_velocity_multiplier, geometry.output_interval_m, candidate_configuration);
            if (trajectory.samples.empty())
            {
                return std::nullopt;
            }
            ...
            for (const auto& sample : trajectory.samples)
            {
                maximum = std::max(maximum, line_of_sight_path_m(sample, geometry.sight_height_m, sight_line_elevation_rad));
            }
            return maximum;
        };
        ...
        const auto trajectory = integrate_trajectory(projectile, scenario.environment, geometry.maximum_distance_m, scenario.muzzle_velocity_multiplier, geometry.output_interval_m, configuration);
        if (trajectory.samples.size() < 2)
        {
            return { 0.0, 0.0, MpbrStatus::horizon_limited };
        }
        ...
        std::size_t apex = 0;
        auto apex_path = line_of_sight_path_m(trajectory.samples.front(), geometry.sight_height_m, sight_line_elevation_rad);
        for (std::size_t index = 1; index < trajectory.samples.size(); ++index)
        {
            const auto path = line_of_sight_path_m(trajectory.samples[index], geometry.sight_height_m, sight_line_elevation_rad);
            if (path > apex_path)
            {
                apex = index;
                apex_path = path;
    ...
    DragValidity evaluate_drag_validity(const Projectile& p, const Trajectory& trajectory, double maximum_distance)
    {
        DragValidity result;
        const auto* sphere = sphere_drag(p);
        const auto* tabulated = tabulated_drag(p);
        if ((!sphere && !tabulated) || trajectory.samples.empty() || !std::isfinite(maximum_distance) || maximum_distance < 0.0)
        {
            return result;
        }
        ...
        const auto outside_mach = observed_mach_min < *result.supported_mach_min || observed_mach_max > *result.supported_mach_max;
        const auto outside_reynolds =
            result.supported_reynolds_min && result.supported_reynolds_max && (observed_reynolds_min < *result.supported_reynolds_min || observed_reynolds_max > *result.supported_reynolds_max);
