...
namespace
{
    ...
    struct Residuals
    {
        ...
        void include(const Residuals& other)
        {
    ...
    const Value::Object& object_field(const Value::Object& object, std::string_view name)
    {
        return field(object, name).as_object();
    }

    const Value::Array& array_field(const Value::Object& object, std::string_view name)
    {
        return field(object, name).as_array();
    }

    double number_field(const Value::Object& object, std::string_view name)
    {
        return field(object, name).as_number();
    }

    const std::string& string_field(const Value::Object& object, std::string_view name)
    {
    ...
    std::vector<G1Band> read_g1_bands(const std::filesystem::path& path)
    {
        ...
        while (std::getline(input, line))
        {
            ...
            std::string exponent;
            if (!std::getline(row, threshold, ',') || !std::getline(row, coefficient, ',') || !std::getline(row, exponent, ','))
            {
    ...
    std::vector<PublishedVelocity> read_manufacturer_velocities(const std::filesystem::path& path)
    {
        std::ifstream input(path);
        if (!input)
        {
        ...
        while (std::getline(input, line))
        {
            ...
            samples.push_back(imperial ? PublishedVelocity { parsed_distance * 0.9144, parsed_velocity * ballistics::fps_to_mps } : PublishedVelocity { parsed_distance, parsed_velocity });
        }
        if (samples.empty())
        {
    ...
    std::vector<AtmosphereDensitySample> read_atmosphere_densities(const std::filesystem::path& path)
    {
        std::ifstream input(path);
        if (!input)
        {
        ...
        while (std::getline(input, line))
        {
            ...
            samples.push_back({ std::stod(cells[0]), std::stod(cells[1]), std::stod(cells[2]), std::stod(cells[4]) });
        }
        if (samples.empty())
        {
    ...
    std::vector<AtmosphereSoundSpeedSample> read_atmosphere_sound_speeds(const std::filesystem::path& path)
    {
        std::ifstream input(path);
        if (!input)
        {
        ...
        while (std::getline(input, line))
        {
            ...
            samples.push_back({ std::stod(cells[0]), std::stod(cells[1]), std::stod(cells[2]), std::stod(cells[3]), std::stod(cells[4]), std::stod(cells[5]), std::stod(cells[6]) });
        }
        if (samples.empty())
        {
    ...
    std::vector<AtmosphereViscositySample> read_atmosphere_viscosities(const std::filesystem::path& path)
    {
        std::ifstream input(path);
        if (!input)
        {
    ...
    double relative_residual(double actual, double expected)
    {
        return std::abs(actual - expected) / std::max(std::abs(expected), 1e-15);
    }

    double g1_expected_cd(const G1Band& band, double speed_fps)
    {
        ...
        const auto retardation_mps2 = band.coefficient * std::pow(speed_fps, band.exponent) * ballistics::fps_to_mps;
        return retardation_mps2 / (0.5 * standard_density_kg_m3 * reference_area_m2 * speed_mps * speed_mps / pounds_to_kg);
    }

    bool nearly_equal(double actual, double expected, double relative_tolerance = 1e-12)
    {
        return relative_residual(actual, expected) <= relative_tolerance;
    }

    void record_failure(std::vector<std::string>& failures, const std::string& label, double actual, double expected, double tolerance)
    {
        std::ostringstream message;
        message << std::setprecision(15) << label << ": expected " << expected << ", got " << actual << ", tolerance " << tolerance;
    ...
    void write_report(const std::filesystem::path& path, const std::string& scenario_set_id, bool passed, const Residuals& maximum_residuals, const std::vector<ScenarioReport>& scenarios)
    {
    ...
    void write_manufacturer_report(const std::filesystem::path& path, bool passed, const std::vector<ManufacturerLoadReport>& loads)
    {
        if (!path.parent_path().empty())
        {
            std::filesystem::create_directories(path.parent_path());
        ...
        for (std::size_t load_index = 0; load_index < loads.size(); ++load_index)
        {
            ...
            output << "    {\n"
                   << "      \"loadId\": \"" << load.load_id << "\",\n"
                   << "      \"sourceDatasetIds\": [";
            for (std::size_t source_index = 0; source_index < load.source_dataset_ids.size(); ++source_index)
            {
                output << "\"" << load.source_dataset_ids[source_index] << "\"" << (source_index + 1 == load.source_dataset_ids.size() ? "" : ", ");
            }
            output << "],\n"
    ...
    void write_atmosphere_report(const std::filesystem::path& path, bool passed, double density_tolerance_relative, double maximum_density_relative,
        const std::vector<AtmosphereDensitySample>& density_samples, double sound_speed_tolerance_relative, double maximum_sound_speed_relative,
        const std::vector<AtmosphereSoundSpeedSample>& sound_speed_samples, double viscosity_tolerance_relative, double maximum_viscosity_relative,
        const std::vector<AtmosphereViscositySample>& viscosity_samples)
    {
        ...
        output
            ...
            << "    \"passed\": " << (maximum_density_relative <= density_tolerance_relative ? "true" : "false") << ",\n"
            << "    \"samples\": [\n";
        ...
        output
            ...
            << "    \"passed\": " << (maximum_sound_speed_relative <= sound_speed_tolerance_relative ? "true" : "false") << ",\n"
            << "    \"samples\": [\n";
        for (std::size_t index = 0; index < sound_speed_samples.size(); ++index)
        {
            ...
            output
                ...
                << "        \"referenceStandardUncertaintyMps\": " << sample.reference_standard_uncertainty_mps << ",\n"
        ...
        output
            ...
            << "    \"passed\": " << (maximum_viscosity_relative <= viscosity_tolerance_relative ? "true" : "false") << ",\n"
            << "    \"samples\": [\n";
...
int main(int argc, char** argv)
{
    ...
    try
    {
        const auto g1_bands = read_g1_bands(source_directory / "validation/sources/g1-gnu-ballistics.csv");
        if (g1_bands.size() != 39)
        {
        ...
        for (std::size_t index = 0; index < g1_bands.size(); ++index)
        {
            ...
            const auto upper_threshold = index == 0 ? band.threshold_fps + 200.0 : g1_bands[index - 1].threshold_fps;
            ...
            {
                record_failure(failures, "G1 segment " + std::to_string(index), actual_segment_cd, expected_segment_cd, 1e-12);
            }
            ...
            if (band.threshold_fps > 0.0)
            {
                ...
                {
                    record_failure(failures, "G1 upper boundary " + std::to_string(index), actual_upper_cd, expected_upper_cd, 1e-12);
                }

                if (index + 1 < g1_bands.size())
                {
                    const auto lower_speed_fps = band.threshold_fps - boundary_offset_fps;
                    const auto lower_mach = lower_speed_fps * fps_to_mps / standard_speed_of_sound_mps;
                    const auto expected_lower_cd = g1_expected_cd(g1_bands[index + 1], lower_speed_fps);
                    const auto actual_lower_cd = g1_drag_coefficient(lower_mach);
                    if (!nearly_equal(actual_lower_cd, expected_lower_cd))
                    {
                        record_failure(failures, "G1 lower boundary " + std::to_string(index), actual_lower_cd, expected_lower_cd, 1e-12);
                    }
                }
            }
        }
        ...
        const auto inventory = json::parse(read_text(source_directory / "validation/normalized/builtin-loads.json"));
        ...
        for (std::size_t index = 0; index < std::min(inventory_loads.size(), built_ins.size()); ++index)
        {
            ...
            if (string_field(record, "id") != load.provenance.id || !nearly_equal(number_field(implementation, "massKg"), load.definition.mass_kg) ||
                !nearly_equal(number_field(implementation, "muzzleVelocityMps"), load.ammunition.muzzle_velocity_mps) || number_field(implementation, "pelletCount") != load.ammunition.payload_count)
            {
                failures.push_back("built-in inventory mismatch for index " + std::to_string(index));
            }
        }

        const auto artifact = json::parse(read_text(source_directory / "validation/scenarios/g7-independent.json"));
        ...
        const auto atmosphere = Atmosphere::create(number_field(atmosphere_json, "temperatureC"),
        ...
        for (const auto& check_value : array_field(root, "aerodynamicChecks"))
        {
            ...
            projectile.definition.drag = ReferenceBcDrag { ReferenceDragCurve::g7, ConstantBallisticCoefficient { number_field(check, "ballisticCoefficient") } };
            const auto speed_mps = mach * atmosphere.speed_of_sound_mps;
            const auto actual_cd = g7_drag_coefficient(mach);
            const auto actual_acceleration = drag_retardation_mps2(speed_mps, projectile, atmosphere);
            if (!nearly_equal(actual_cd, expected_cd))
            {
                record_failure(failures, "G7 independent Cd Mach " + std::to_string(mach), actual_cd, expected_cd, 1e-12);
            }
            if (!nearly_equal(actual_acceleration, expected_acceleration, 2e-12))
            {
                record_failure(failures, "G7 independent acceleration Mach " + std::to_string(mach), actual_acceleration, expected_acceleration, 2e-12);
            }
        }
        ...
        for (const auto& scenario_value : array_field(root, "scenarios"))
        {
            ...
            projectile.definition.drag = ReferenceBcDrag { ReferenceDragCurve::g7, ConstantBallisticCoefficient { number_field(scenario, "ballisticCoefficient") } };
            const auto maximum_distance_m = number_field(scenario, "maximumDistanceM");
            const auto trajectory = integrate_trajectory(projectile, atmosphere, maximum_distance_m);
            ...
            for (const auto& sample_value : array_field(scenario, "samples"))
            {
                ...
                if (!actual)
                {
                    failures.push_back(report.id + " has no production sample at " + std::to_string(distance_m) + " m");
                    report.passed = false;
                    continue;
                }
                Residuals residual { relative_residual(actual->ground_speed_mps, number_field(expected, "speedMps")),
                ...
                if (residual.velocity_relative > velocity_tolerance || residual.time_relative > time_tolerance || residual.position_absolute_m > position_tolerance)
                {
                    report.passed = false;
                }
            }
            if (!report.passed)
            {
                failures.push_back(report.id + " exceeds its declared independent-reference tolerance");
            }
            overall_maximum.include(report.maximum_residuals);
            scenario_reports.push_back(report);
        }

        write_report(report_path, scenario_set_id, failures.empty(), overall_maximum, scenario_reports);

        constexpr double atmosphere_property_tolerance = 0.001;
        auto density_samples = read_atmosphere_densities(source_directory / "validation/sources/atmosphere-cipm-2007.csv");
        ...
        {
            const auto production = Atmosphere::create(sample.temperature_c, sample.station_pressure_hpa, sample.relative_humidity_percent, 0.0, 0.0);
            sample.production_density_kg_m3 = production.density_kg_m3;
            sample.density_relative_residual = relative_residual(sample.production_density_kg_m3, sample.reference_density_kg_m3);
            maximum_atmosphere_density_residual = std::max(maximum_atmosphere_density_residual, sample.density_relative_residual);
            if (sample.density_relative_residual > atmosphere_property_tolerance)
            {
                record_failure(failures,
                    "atmosphere density at " + std::to_string(sample.temperature_c) + " C, " + std::to_string(sample.station_pressure_hpa) + " hPa, " +
        ...
        auto sound_speed_samples = read_atmosphere_sound_speeds(source_directory / "validation/sources/atmosphere-sound-speed-gavioso-2025.csv");
        ...
        for (auto& sample : sound_speed_samples)
        {
            ...
            sample.speed_of_sound_relative_residual = relative_residual(sample.production_speed_of_sound_mps, sample.reference_speed_of_sound_mps);
            maximum_atmosphere_sound_speed_residual = std::max(maximum_atmosphere_sound_speed_residual, sample.speed_of_sound_relative_residual);
            if (sample.speed_of_sound_relative_residual > atmosphere_property_tolerance)
            {
                record_failure(failures,
                    "atmosphere sound speed at " + std::to_string(sample.temperature_c) + " C, " + std::to_string(sample.station_pressure_hpa) + " hPa, " +
                        std::to_string(sample.relative_humidity_percent) + "% RH",
        ...
        auto viscosity_samples = read_atmosphere_viscosities(source_directory / "validation/sources/air-viscosity-naca-1135.csv");
        double maximum_atmosphere_viscosity_residual = 0.0;
        for (auto& sample : viscosity_samples)
        {
            const auto production = Atmosphere::create(sample.temperature_c, 1013.25, 0.0, 0.0, 0.0);
            sample.production_dynamic_viscosity_pa_s = production.dynamic_viscosity_pa_s;
            sample.dynamic_viscosity_relative_residual = relative_residual(sample.production_dynamic_viscosity_pa_s, sample.reference_dynamic_viscosity_pa_s);
            maximum_atmosphere_viscosity_residual = std::max(maximum_atmosphere_viscosity_residual, sample.dynamic_viscosity_relative_residual);
        ...
        write_atmosphere_report(atmosphere_report_path,
            atmosphere_passed,
            atmosphere_property_tolerance,
        ...
        const std::vector<ManufacturerDefinition> manufacturer_definitions {
            { 0,
                { "validation/sources/bp-white-blackout-hv-hunting-spot-2018.csv", "validation/sources/bp-white-blackout-hv-caccia-magazine-2019.csv" },
                { "manufacturer-attributed-bp-white-blackout-hv-2018", "secondary-bp-white-blackout-hv-caccia-magazine-2019" },
            ...
            { 1,
                ...
                "fitted_to_same_table",
                0.01 },
            { 2, { "validation/sources/winchester-x123rs15-2026.csv" }, { "manufacturer-winchester-x123rs15-2026" }, "primary_manufacturer_publication", "manufacturer_published", 0.005 },
            { 3, { "validation/sources/hornady-80971-2022.csv" }, { "manufacturer-hornady-80971-2022" }, "primary_manufacturer_publication", "manufacturer_published", 0.005 },
            { 4, { "validation/sources/federal-308a-2022.csv" }, { "manufacturer-federal-308a-2022" }, "primary_manufacturer_publication", "fitted_to_same_table", 0.005 },
        };
        ...
        for (const auto& definition : manufacturer_definitions)
        {
            ...
            {
                const auto file_samples = read_manufacturer_velocities(source_directory / source_path);
                source_samples.insert(source_samples.end(), file_samples.begin(), file_samples.end());
            }
            std::ranges::sort(source_samples, {}, &PublishedVelocity::distance_m);
            const auto maximum_distance_m = source_samples.back().distance_m;
            const auto trajectory = integrate_trajectory(built_ins[definition.built_in_index], manufacturer_atmosphere, maximum_distance_m);
            ...
            for (const auto& published : source_samples)
            {
                ...
                if (!actual)
                {
                    failures.push_back(report.load_id + " has no sample at manufacturer distance " + std::to_string(published.distance_m) + " m");
                    report.passed = false;
                    continue;
                }
                const auto residual = relative_residual(actual->ground_speed_mps, published.velocity_mps);
                report.maximum_velocity_relative = std::max(report.maximum_velocity_relative, residual);
                report.samples.push_back({ published.distance_m, published.velocity_mps, actual->ground_speed_mps, residual });
            }
            if (report.maximum_velocity_relative > report.tolerance_velocity_relative)
            {
                failures.push_back(report.load_id + " exceeds its manufacturer-table velocity tolerance");
                report.passed = false;
            }
            manufacturer_reports.push_back(std::move(report));
        }
        const auto manufacturer_passed = failures.size() == manufacturer_failure_start;
        write_manufacturer_report(manufacturer_report_path, manufacturer_passed, manufacturer_reports);
    }
