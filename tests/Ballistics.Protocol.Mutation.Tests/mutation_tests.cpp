...
namespace
{
    ...
    [[noreturn]] void fail(std::string_view message, std::size_t case_index, std::string_view source)
    {
        std::cerr << "Protocol mutation property failed in case " << case_index << ": " << message << "\nInput bytes: " << source.size() << "\n";
        std::exit(1);
    }

    void expect(bool condition, std::string_view message, std::size_t case_index, std::string_view source)
    {
        if (!condition)
        {
    ...
    public:
        ...
        [[nodiscard]] std::size_t index(std::size_t upper_bound)
        {
            if (upper_bound == 0)
            {
                return 0;
  ...
  "customLoads": [{
    ...
    void replace_random_byte(std::string& source, DeterministicGenerator& generator)
    {
        if (source.empty())
        {
    ...
    void insert_random_bytes(std::string& source, DeterministicGenerator& generator)
    {
    ...
    void erase_random_range(std::string& source, DeterministicGenerator& generator)
    {
        if (source.empty())
        {
            return;
    ...
    void duplicate_random_range(std::string& source, DeterministicGenerator& generator)
    {
    ...
    void truncate(std::string& source, DeterministicGenerator& generator)
    {
        if (!source.empty())
        {
    ...
    void insert_structured_token(std::string& source, DeterministicGenerator& generator)
    {
    ...
    void replace_ascii_digit(std::string& source, DeterministicGenerator& generator)
    {
    ...
    void mutate(std::string& source, DeterministicGenerator& generator)
    {
    ...
    void verify_error_envelope(std::string_view response, std::size_t case_index, std::string_view source)
    {
        expect(response.size() < 256 * 1024, "error response remained bounded", case_index, source);
        expect(!response.empty() && response.back() == '\n', "error response ends with newline", case_index, source);
        try
        {
            ...
            expect(ok != object.end() && ok->second.is_bool() && !ok->second.as_bool(), "error response carries ok=false", case_index, source);
            expect(version != object.end() && version->second.is_number() && version->second.as_number() == ballistics::protocol::current_version,
                "error response carries the current protocol version",
                case_index,
                source);
            expect(issues != object.end() && issues->second.is_array() && !issues->second.as_array().empty(), "error response carries issues", case_index, source);
        }
    ...
    bool verify_parse(std::string_view source, std::size_t case_index)
    {
        ...
        expect(parsed.request.has_value() == parsed.issues.empty(), "accepted requests have no parse issues and rejected requests have issues", case_index, source);
        if (parsed.request)
        {
            const auto& request = *parsed.request;
            expect(request.protocol_version == ballistics::protocol::current_version, "accepted request uses the current protocol version", case_index, source);
            expect(!request.request_id.empty() && request.request_id.size() <= 128, "accepted request ID is bounded", case_index, source);
            expect(request.custom_loads.size() <= 3, "accepted custom-load collection is bounded", case_index, source);
            return true;
        }
        ...
        for (const auto& issue : parsed.issues)
        {
            ...
            expect(issue.severity == ballistics::ValidationSeverity::error, "protocol parse rejections are errors", case_index, source);
        }
        verify_error_envelope(ballistics::protocol::error_response(parsed.request_id, parsed.issues), case_index, source);
        return false;
    }
    ...
    void verify_oversize_rejection()
    {
        ...
        expect(!parsed.request && parsed.issues.size() == 1, "oversize request is rejected once", 0, oversized);
        expect(parsed.issues[0].code == "protocol.request.too_large", "oversize rejection uses the stable issue code", 0, oversized);
    }
...
int main()
{
    ...
    for (std::size_t iteration = 0; iteration < mutation_iterations; ++iteration)
    {
        ...
        std::string source = corpus_index == 0 ? std::string(valid_request) : std::string(malformed_corpus[corpus_index - 1]);
    ...
    expect(rejected > mutation_iterations / 2, "mutation suite exercises rejection paths", case_index, {});
    std::cout << "Protocol mutation properties passed: " << case_index << " cases (" << accepted << " accepted, " << rejected << " rejected), seed 0x8e5f17a4c93d620b.\n";
    return 0;
}
