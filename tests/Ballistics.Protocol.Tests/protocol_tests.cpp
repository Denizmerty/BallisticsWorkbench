...
namespace
{
  ...
  "observations": [
    ...
    std::string request_with_custom_load_name(std::string name)
    {
...
int main()
{
    ...
    expect_true("G7 discriminant parsed", drag_model(parsed.request->custom_loads[0]) == DragModel::g7);
    expect_true("sphere discriminant parsed", drag_model(parsed.request->custom_loads[1]) == DragModel::sphere);
    expect_true("Unicode escape decoded",
    ...
    const auto maximum_unicode_request = protocol::parse_request(request_with_custom_load_name(maximum_unicode_name));
    expect_true("120-byte Unicode load name accepted", maximum_unicode_request.request && maximum_unicode_request.request->custom_loads[0].definition.name.size() == 120);
    maximum_unicode_name += "\xc3\x87";
    const auto oversized_unicode_request = protocol::parse_request(request_with_custom_load_name(maximum_unicode_name));
    expect_true("122-byte Unicode load name rejected",
        !oversized_unicode_request.request &&
            std::any_of(oversized_unicode_request.issues.begin(),
                oversized_unicode_request.issues.end(),
                [](const auto& issue)
                {
                    return issue.code == "custom_load.name.length";
                }));

    auto maximum_request = std::string(valid_request);
    maximum_request.append(protocol::maximum_request_bytes - maximum_request.size(), ' ');
    expect_true("exact maximum-byte request accepted", protocol::parse_request(maximum_request).request.has_value());
    std::istringstream maximum_stream(maximum_request);
    expect_true("standard input preserves exact maximum-byte request", cli::read_standard_input(maximum_stream).size() == protocol::maximum_request_bytes);
    ...
    expect_true("standard input retains enough bytes for oversize rejection",
        oversized_input.size() > protocol::maximum_request_bytes && protocol::parse_request(oversized_input).issues[0].code == "protocol.request.too_large");
    expect_true("uncertainty block parsed",
        parsed.request->scenario.uncertainty.has_value() && parsed.request->scenario.uncertainty->rifle_muzzle_velocity_standard_deviation_mps == 3.0 &&
            parsed.request->scenario.uncertainty->drag_relative_standard_deviation == 0.02 && parsed.request->scenario.uncertainty->rifle_zero_range_standard_deviation_m == 1.5);
    expect_true("buckshot pattern evidence parsed",
        parsed.request->scenario.buckshot_pattern.has_value() && parsed.request->scenario.buckshot_pattern->load_id == "custom:sphere" &&
    ...
    negative_uncertainty.replace(drag_sd, std::string("\"dragRelativeStandardDeviation\": 0.02").size(), "\"dragRelativeStandardDeviation\": -0.02");
    const auto invalid_uncertainty = protocol::parse_request(negative_uncertainty);
    expect_true("negative uncertainty rejected", !invalid_uncertainty.request && !invalid_uncertainty.issues.empty());

    auto unknown_uncertainty = std::string(valid_request);
    const auto uncertainty_end = unknown_uncertainty.find("\n    }", unknown_uncertainty.find("\"uncertainty\""));
    ...
    expect_true("unknown uncertainty field rejected",
        !invalid_uncertainty_field.request &&
            std::any_of(invalid_uncertainty_field.issues.begin(),
                invalid_uncertainty_field.issues.end(),
                [](const auto& issue)
                {
                    return issue.code == "protocol.field.unknown";
                }));

    const auto calibration = protocol::parse_request(valid_calibration_request);
    expect_true("calibration request accepted", calibration.request.has_value() && calibration.issues.empty());
    expect_true("calibration operation parsed", calibration.request->operation == protocol::RequestOperation::calibrate_reference_bc);
    expect_true("calibration curve and fit parsed",
        drag_model(calibration.request->calibration.projectile) == DragModel::g7 && calibration.request->calibration.fit_kind == BcFitKind::velocity_bands &&
            calibration.request->calibration.band_minimum_velocities_mps.size() == 2);
    expect_true("calibration observation roles and uncertainty parsed",
        calibration.request->calibration.observations.size() == 4 && calibration.request->calibration.observations[0].standard_deviation_mps == 1.0 &&
    ...
    unordered_observations.replace(third_distance, std::string("\"distanceM\": 300").size(), "\"distanceM\": 150");
    const auto invalid_observation_order = protocol::parse_request(unordered_observations);
    expect_true("unordered calibration observations rejected",
        !invalid_observation_order.request &&
            std::any_of(invalid_observation_order.issues.begin(),
                invalid_observation_order.issues.end(),
                [](const auto& issue)
                {
                    return issue.code == "calibration.observations.order";
                }));

    auto insufficient_observations = std::string(valid_calibration_request);
    const auto second_role = insufficient_observations.find("\"role\": \"calibration\"", insufficient_observations.find("\"distanceM\": 200"));
    insufficient_observations.replace(second_role, std::string("\"role\": \"calibration\"").size(), "\"role\": \"holdout\"");
    const auto third_role = insufficient_observations.find("\"role\": \"calibration\"", insufficient_observations.find("\"distanceM\": 300"));
    insufficient_observations.replace(third_role, std::string("\"role\": \"calibration\"").size(), "\"role\": \"holdout\"");
    const auto invalid_information = protocol::parse_request(insufficient_observations);
    expect_true("too few calibration observations rejected",
        !invalid_information.request &&
            std::any_of(invalid_information.issues.begin(),
    ...
    expect_true("calibration bands must begin at zero",
        !invalid_band_coverage.request &&
            std::any_of(invalid_band_coverage.issues.begin(),
                invalid_band_coverage.issues.end(),
                [](const auto& issue)
                {
                    return issue.code == "calibration.fit.band_coverage";
                }));

    auto banded_text = std::string(valid_request);
    const auto scalar_bc = banded_text.find("\"ballisticCoefficient\": 0.25");
    banded_text.replace(scalar_bc,
    ...
    const auto* parsed_reference = banded.request ? reference_bc_drag(banded.request->custom_loads[0]) : nullptr;
    expect_true("velocity-banded request accepted", parsed_reference && std::holds_alternative<BandedBallisticCoefficient>(parsed_reference->coefficient));
    const auto& parsed_bands = std::get<BandedBallisticCoefficient>(parsed_reference->coefficient).bands;
    expect_true("velocity-band values preserved", parsed_bands[1].minimum_velocity_mps == 400.0 && parsed_bands[1].ballistic_coefficient == 0.25);
    ...
    tabulated_text.replace(drag_start, drag_end - drag_start + std::string("\n      }").size(), tabulated_drag);
    const auto tabulated = protocol::parse_request(tabulated_text);
    expect_true("tabulated Mach-Cd request accepted", tabulated.request && drag_model(tabulated.request->custom_loads[0]) == DragModel::tabulated_cd);
    const auto* parsed_tabulated = tabulated.request ? ballistics::tabulated_drag(tabulated.request->custom_loads[0]) : nullptr;
    expect_true("tabulated Mach-Cd values preserved",
        parsed_tabulated && parsed_tabulated->reference_diameter_m == 0.00782 && parsed_tabulated->points.size() == 3 && parsed_tabulated->points[1].drag_coefficient == 0.4);
    ...
    expect_true("unordered Mach-Cd curve rejected",
        !invalid_curve.request &&
            std::any_of(invalid_curve.issues.begin(),
    ...
    both_bc_forms.insert(bc_insertion,
    ...
    expect_true("scalar and banded BC are mutually exclusive",
        !mutually_exclusive.request &&
            std::any_of(mutually_exclusive.issues.begin(),
                mutually_exclusive.issues.end(),
                [](const auto& issue)
                {
                    return issue.code == "protocol.field.mutually_exclusive";
                }));

    auto unordered_bands = banded_text;
    const auto threshold = unordered_bands.find("\"minimumVelocityMps\":400");
    unordered_bands.replace(threshold, std::string("\"minimumVelocityMps\":400").size(), "\"minimumVelocityMps\":0");
    const auto invalid_schedule = protocol::parse_request(unordered_bands);
    expect_true("unordered velocity bands rejected",
        !invalid_schedule.request &&
            std::any_of(invalid_schedule.issues.begin(),
    ...
    const auto duplicate = protocol::parse_request(R"({"protocolVersion":2,"protocolVersion":2,"requestId":"x"})");
    expect_true("duplicate JSON key rejected", !duplicate.request && !duplicate.issues.empty() && duplicate.issues[0].code == "protocol.json.invalid");
    ...
    expect_true("invalid UTF-8 rejected", !invalid_utf8.request && !invalid_utf8.issues.empty() && invalid_utf8.issues[0].code == "protocol.json.invalid");
    ...
    const auto response = protocol::error_response("request\"id", { { "test.code", "test.field", "test message", ValidationSeverity::error } });
    expect_true("error response includes protocol version", response.find("\"protocolVersion\":" + std::to_string(protocol::current_version)) != std::string::npos);
    expect_true("error response includes model identity",
        response.find("\"engineVersion\":\"" + std::string(engine_version) + "\"") != std::string::npos &&
            response.find("\"modelVersion\":\"" + std::string(model_version) + "\"") != std::string::npos);
    expect_true("error response escapes request ID", response.find("\"requestId\":\"request\\\"id\"") != std::string::npos);
