...
namespace
{
    ...
    BW_TEST_CASE("published Cramer sound-speed cases are parameterized")
    {
        struct SoundCase
        {
        ...
        for_each_case<SoundCase>(cases,
            [](const SoundCase& sample)
            {
                const auto atmosphere = Atmosphere::create(sample.temperature_c, sample.pressure_hpa, sample.humidity_percent, 0.0, 0.0);
                expect_near(atmosphere.speed_of_sound_mps, sample.reference_mps, sample.conformance_tolerance_mps, "published sound speed");
                expect(ballistics::atmosphere_model_validity(atmosphere).sound_speed_within_declared_domain == sample.declared_in_domain, "sound-speed validity qualification");
            });
    }

    BW_TEST_CASE("atmosphere validity flags follow the checked source domains")
    {
        const auto qualified = ballistics::Atmosphere::create(20.0, 1013.25, 50.0, 0.0, 0.0);
        const auto density_temperature_extrapolation = ballistics::Atmosphere::create(10.0, 1013.25, 50.0, 0.0, 0.0);
        const auto density_pressure_extrapolation = ballistics::Atmosphere::create(20.0, 599.0, 50.0, 0.0, 0.0);
        const auto sound_pressure_extrapolation = ballistics::Atmosphere::create(20.0, 1030.0, 50.0, 0.0, 0.0);

        expect(ballistics::atmosphere_model_validity(qualified).density_within_declared_domain, "CIPM-qualified density point");
        expect(!ballistics::atmosphere_model_validity(density_temperature_extrapolation).density_within_declared_domain, "density temperature extrapolation");
        expect(!ballistics::atmosphere_model_validity(density_pressure_extrapolation).density_within_declared_domain, "density pressure extrapolation");
        expect(!ballistics::atmosphere_model_validity(sound_pressure_extrapolation).sound_speed_within_declared_domain, "sound-speed pressure extrapolation");
    }

    BW_TEST_CASE("WGS-84 local gravity varies by latitude and altitude")
    {
        struct GravityCase
        {
            double latitude_deg;
        ...
        for_each_case<GravityCase>(cases,
            [](const GravityCase& sample)
            {
                ...
                expect_near(environment.gravity_at_height(0.0), sample.expected_mps2, 5e-10, "WGS-84 gravity");
            });
    }

    BW_TEST_CASE("wind layers interpolate on their declared coordinate axis")
    {
        ...
        constexpr std::array cases {
            WindCase { { 0, 0, 0 }, 1, 2 },
            WindCase { { 100, 0, 0 }, 2, -4 },
            WindCase { { 200, 0, 0 }, 6, 0 },
            WindCase { { 300, 0, 0 }, 10, 4 },
            WindCase { { 50, 30, 0 }, -6, 7 },
        };
        for_each_case<WindCase>(cases,
    ...
    BW_TEST_CASE("Coriolis acceleration follows latitude and azimuth sign conventions")
    {
        ...
        const std::array cases {
            ...
            CoriolisCase {
                ...
                90.0,
                { 800, 0, 0 },
                { 0, 2.0 * earth_rotation * std::cos(std::numbers::pi / 4.0) * 800, 2.0 * earth_rotation * std::sin(std::numbers::pi / 4.0) * 800 },
            },
        };
        for_each_case<CoriolisCase>(cases,
    ...
    BW_TEST_CASE("altitude-dependent atmosphere has lower pressure and density aloft")
    {
        ...
        expect(aloft.station_pressure_hpa < firing_point.station_pressure_hpa, "pressure decreases with height");
        expect(aloft.density_kg_m3 < firing_point.density_kg_m3, "density decreases with height");
    }

    BW_TEST_CASE("empirical buckshot analysis separates calibration and holdout evidence")
    {
        ...
        input.target = {
            ballistics::TargetRegionShape::circle,
            0.30,
            0.30,
            0.0,
            0.0,
        ...
        expect(result.status == ballistics::PatternAnalysisStatus::validated_in_domain, "target lies inside calibration range");
        expect(result.holdout_rmse_m.has_value(), "holdout RMSE is separate");
        expect(result.residuals.back().role == ballistics::ObservationRole::holdout, "holdout retained");
        ...
        expect(result.per_pellet_hit_probability > 0.0 && result.per_pellet_hit_probability < 1.0, "per-pellet probability bounded");
        expect_near(std::accumulate(result.pellet_count_probabilities.begin(), result.pellet_count_probabilities.end(), 0.0), 1.0, 1e-12, "pellet-count distribution normalized");
        expect(result.validity_statement.find("Pellet wakes") != std::string::npos, "swarm-aerodynamics exclusion stated");
    }

    BW_TEST_CASE("buckshot analysis flags range extrapolation")
    {
        ballistics::BuckshotPatternInput input;
        ...
        input.target = {
            ballistics::TargetRegionShape::rectangle,
            0.4,
            0.6,
            0.1,
            -0.1,
        ...
        const auto result = ballistics::analyze_buckshot_pattern(input);
        expect(result.status == ballistics::PatternAnalysisStatus::extrapolated, "outside calibration range is explicit");
        expect(result.probability_at_least_minimum >= 0.0 && result.probability_at_least_minimum <= 1.0, "at-least probability bounded");
    }
