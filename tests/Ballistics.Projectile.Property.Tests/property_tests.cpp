...
namespace
{
    ...
    public:
        ...
        std::size_t count(std::size_t minimum, std::size_t maximum)
        {
    ...
    bool contains_code(const std::vector<ballistics::ValidationIssue>& issues, std::string_view code)
    {
    ...
    void expect(bool condition, std::string_view message, std::size_t& failures)
    {
        if (!condition)
        {
            ++failures;
    ...
    ballistics::Projectile base_load(std::size_t sequence)
    {
    ...
    void check_valid_reference_cases(DeterministicRandom& random, std::size_t& failures)
    {
        for (std::size_t iteration = 0; iteration < 3000; ++iteration)
        {
            auto load = base_load(iteration);
            const auto curve = iteration % 2 == 0 ? ballistics::ReferenceDragCurve::g1 : ballistics::ReferenceDragCurve::g7;
            if (iteration % 3 == 0)
            {
                load.definition.drag = ballistics::ReferenceBcDrag { curve, ballistics::ConstantBallisticCoefficient { random.between(0.005, 2.0) } };
            }
            else
            {
            ...
            expect(ballistics::reference_bc_drag(load) != nullptr && ballistics::tabulated_drag(load) == nullptr && ballistics::sphere_drag(load) == nullptr,
            ...
            expect(std::isfinite(coefficient) && coefficient >= 0.005 && coefficient <= 2.0, "valid reference coefficient lookup left its domain", failures);
        }
    }

    void check_valid_tabulated_cases(DeterministicRandom& random, std::size_t& failures)
    {
        ...
        for (std::size_t iteration = 0; iteration < 3000; ++iteration)
        {
            ...
            expect(ballistics::tabulated_drag(load) != nullptr && ballistics::reference_bc_drag(load) == nullptr && ballistics::sphere_drag(load) == nullptr,
            ...
            expect(std::isfinite(acceleration) && acceleration > 0.0, "valid tabulated load produced invalid drag", failures);
        }
    }

    void check_valid_sphere_cases(DeterministicRandom& random, std::size_t& failures)
    {
        ...
        for (std::size_t iteration = 0; iteration < 3000; ++iteration)
        {
            ...
            expect(ballistics::sphere_drag(load) != nullptr && ballistics::reference_bc_drag(load) == nullptr && ballistics::tabulated_drag(load) == nullptr,
                "sphere alternative was not exclusive",
                failures);
            const auto acceleration = ballistics::drag_retardation_mps2(random.between(1.0, 500.0), load, atmosphere);
            expect(std::isfinite(acceleration) && acceleration > 0.0, "valid sphere load produced invalid drag", failures);
        }
    }

    void check_invalid_boundaries(std::size_t& failures)
    {
        auto load = base_load(9000);

        for (const auto invalid : { 0.0, -0.1, 2.01, std::numeric_limits<double>::infinity() })
        {
            load.definition.drag = ballistics::ReferenceBcDrag { ballistics::ReferenceDragCurve::g1, ballistics::ConstantBallisticCoefficient { invalid } };
            expect(contains_code(ballistics::validate_projectile(load), "projectile.bc.range"), "invalid constant BC was not rejected", failures);
        }

        load.definition.drag = ballistics::ReferenceBcDrag { ballistics::ReferenceDragCurve::g7, ballistics::BandedBallisticCoefficient { { { 100.0, 0.2 }, { 400.0, 0.3 } } } };
        expect(contains_code(ballistics::validate_projectile(load), "projectile.bc_schedule.coverage"), "uncovered BC schedule was not rejected", failures);
        load.definition.drag = ballistics::ReferenceBcDrag { ballistics::ReferenceDragCurve::g7, ballistics::BandedBallisticCoefficient { { { 0.0, 0.2 }, { 500.0, 0.3 }, { 400.0, 0.4 } } } };
        expect(contains_code(ballistics::validate_projectile(load), "projectile.bc_schedule.order"), "unordered BC schedule was not rejected", failures);

        load.definition.drag = ballistics::TabulatedDrag { 0.0009, { { 0.0, 0.3 }, { 1.0, 0.3 } } };
        expect(contains_code(ballistics::validate_projectile(load), "projectile.mach_cd.diameter_range"), "invalid tabulated diameter was not rejected", failures);
        load.definition.drag = ballistics::TabulatedDrag { 0.01, { { 1.0, 0.3 }, { 0.5, 0.2 } } };
        expect(contains_code(ballistics::validate_projectile(load), "projectile.mach_cd.order"), "unordered Mach-Cd table was not rejected", failures);

        load.definition.drag = ballistics::SphereDrag { 0.01, 100.0 };
        expect(contains_code(ballistics::validate_projectile(load), "projectile.sphere.density_range"), "invalid sphere density was not rejected", failures);
        load.definition.drag = ballistics::SphereDrag { 0.01, 7800.0 };
        load.definition.mass_kg = 1.0;
        expect(contains_code(ballistics::validate_projectile(load), "projectile.sphere.mass_inconsistent"), "inconsistent sphere mass was not rejected", failures);

        load = base_load(9001);
        load.definition.geometry = ballistics::ProjectileGeometry { 0.0, 0.308 };
        expect(contains_code(ballistics::validate_projectile(load), "projectile.geometry.positive"), "invalid optional geometry was not rejected", failures);
        load = base_load(9002);
        load.ammunition.payload_count = 0;
        expect(contains_code(ballistics::validate_projectile(load), "projectile.payload_count.range"), "invalid payload count was not rejected", failures);
    }
