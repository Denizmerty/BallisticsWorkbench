...
namespace
{
    ...
    struct ErrorMetrics
    {
        ...
        void include(const ErrorMetrics& other)
        {
    ...
    ErrorMetrics difference(const ballistics::TrajectorySample& actual, const ballistics::TrajectorySample& expected)
    {
        return {
            std::hypot(actual.position_m.y - expected.position_m.y, actual.position_m.z - expected.position_m.z),
            std::hypot(actual.ground_velocity_mps.x - expected.ground_velocity_mps.x,
    ...
    bool within(const ErrorMetrics& actual, const ErrorMetrics& limit)
    {
        return actual.position_m <= limit.position_m && actual.velocity_mps <= limit.velocity_mps && actual.time_s <= limit.time_s;
    }

    bool no_worse_when_tightened(const ErrorMetrics& tighter, const ErrorMetrics& production)
    {
        constexpr ErrorMetrics numerical_floor { 1e-10, 1e-9, 1e-11 };
        return tighter.position_m <= std::max(numerical_floor.position_m, production.position_m * 1.05) &&
            tighter.velocity_mps <= std::max(numerical_floor.velocity_mps, production.velocity_mps * 1.05) && tighter.time_s <= std::max(numerical_floor.time_s, production.time_s * 1.05);
    }

    ballistics::SolverConfiguration scaled_configuration(double scale)
    {
        ballistics::SolverConfiguration configuration;
    ...
    std::vector<double> comparison_distances(double maximum_distance_m)
    {
        ...
        {
            distances.push_back(maximum_distance_m * static_cast<double>(index) / static_cast<double>(intervals));
        }
        return distances;
    }

    ballistics::TrajectorySample sample_at(const ballistics::Trajectory& trajectory, double distance_m, std::string_view label)
    {
        const auto sample = trajectory.sample_at(distance_m);
        if (!sample)
        {
            throw std::runtime_error(std::string(label) + " has no sample at " + std::to_string(distance_m) + " m");
        }
        return *sample;
    }
    ...
    std::vector<ScenarioDefinition> scenario_definitions()
    {
        ...
        g7.definition.drag = ballistics::ReferenceBcDrag { ballistics::ReferenceDragCurve::g7, ballistics::ConstantBallisticCoefficient { 0.24 } };
        ...
        low_velocity.definition.drag = ballistics::TabulatedDrag { 0.008, { { 0.0, 0.22 }, { 0.75, 0.23 }, { 0.95, 0.31 }, { 1.05, 0.38 }, { 1.3, 0.29 }, { 2.0, 0.24 } } };

        return {
            { "g1-standard-calm", loads[3], ballistics::Atmosphere::create(15.0, 1013.25, 50.0, 0.0, 0.0), 2000.0 },
            { "g1-cold-dense-headwind", loads[4], ballistics::Atmosphere::create(-40.0, 1080.0, 20.0, 25.0, -10.0), 1500.0 },
            { "g7-hot-thin-crosswind", g7, ballistics::Atmosphere::create(45.0, 700.0, 15.0, -12.0, 14.0), 2000.0 },
            { "tabulated-low-velocity", low_velocity, ballistics::Atmosphere::create(5.0, 950.0, 80.0, 15.0, 6.0), 1000.0 },
        };
    }

    ScenarioReport evaluate_scenario(const ScenarioDefinition& definition)
    {
        const auto distances = comparison_distances(definition.maximum_distance_m);
        const auto reference = ballistics::integrate_trajectory(definition.projectile, definition.atmosphere, definition.maximum_distance_m, 1.0, 0.25, reference_configuration());
        ...
        const std::array<std::pair<std::string_view, double>, 3> levels { { { "production", 1.0 }, { "half", 0.5 }, { "tenth", 0.1 } } };
        ...
        for (const auto& [id, scale] : levels)
        {
            ...
            auto trajectory = ballistics::integrate_trajectory(definition.projectile, definition.atmosphere, definition.maximum_distance_m, 1.0, 0.25, configuration);
            if (trajectory.termination != ballistics::TrajectoryTermination::requested_distance)
            {
                throw std::runtime_error(definition.id + " " + std::string(id) + " solve did not cover its range");
            }
            ToleranceRow row;
            row.id = id;
            ...
            {
                row.error_to_reference.include(difference(sample_at(trajectory, distance_m, id), sample_at(reference, distance_m, "tight reference")));
            }
        ...
        for (const auto distance_m : distances)
        {
            report.half_tolerance_change.include(difference(sample_at(trajectories[0], distance_m, "production"), sample_at(trajectories[1], distance_m, "half tolerance")));
        }
        ...
        report.tighter_solution_improved = no_worse_when_tightened(report.rows[2].error_to_reference, report.rows[0].error_to_reference);
        return report;
    }

    std::vector<AnalyticStepRow> evaluate_analytic_step_refinement(bool& passed)
    {
        ...
        projectile.definition.drag = ballistics::TabulatedDrag { 0.01, { { 0.0, 0.3 }, { 10.0, 0.3 } } };
        ...
        const auto drag_per_metre = 0.5 * atmosphere.density_kg_m3 * 0.3 * area_m2 / projectile.definition.mass_kg;
        const auto expected_speed_mps = projectile.ammunition.muzzle_velocity_mps * std::exp(-drag_per_metre * distance_m);
        const auto expected_time_s = (std::exp(drag_per_metre * distance_m) - 1.0) / (drag_per_metre * projectile.ammunition.muzzle_velocity_mps);
        ...
        for (const auto maximum_step_s : maximum_steps)
        {
            ...
            const auto trajectory = ballistics::integrate_trajectory(projectile, atmosphere, distance_m, 1.0, 0.25, configuration);
            const auto& sample = sample_at(trajectory, distance_m, "analytic refinement");
            rows.push_back({ maximum_step_s, trajectory.solver.accepted_steps, std::abs(sample.ground_speed_mps - expected_speed_mps), std::abs(sample.time_s - expected_time_s) });
        }
        for (std::size_t index = 1; index < rows.size(); ++index)
        {
            rows[index].observed_speed_order = std::log(rows[index - 1].speed_error_mps / rows[index].speed_error_mps) / std::log(2.0);
            rows[index].observed_time_order = std::log(rows[index - 1].time_error_s / rows[index].time_error_s) / std::log(2.0);
        }
        passed = rows.back().speed_error_mps < rows.front().speed_error_mps && rows.back().time_error_s < rows.front().time_error_s && rows[2].observed_speed_order >= 1.8 &&
            rows[3].observed_speed_order >= 1.8 && rows[2].observed_time_order >= 1.8 && rows[3].observed_time_order >= 1.8;
        return rows;
    }

    void write_metrics(std::ostream& output, const ErrorMetrics& metrics, std::string_view indent)
    {
        output << indent << "{\n"
    ...
    void write_report(const std::filesystem::path& path, bool passed, const ErrorMetrics& maximum_production_error, const ErrorMetrics& maximum_half_change, bool analytic_passed,
        const std::vector<AnalyticStepRow>& analytic_rows, const std::vector<ScenarioReport>& scenarios)
    {
        ...
        for (std::size_t scenario_index = 0; scenario_index < scenarios.size(); ++scenario_index)
        {
            ...
            output
                ...
                << "      \"productionBudgetPassed\": " << (scenario.production_budget_passed ? "true" : "false") << ",\n"
                << "      \"halfToleranceDisplayPassed\": " << (scenario.half_tolerance_display_passed ? "true" : "false") << ",\n"
                << "      \"tighterSolutionImproved\": " << (scenario.tighter_solution_improved ? "true" : "false") << ",\n"
                << "      \"halfToleranceChange\": ";
            ...
            for (std::size_t row_index = 0; row_index < scenario.rows.size(); ++row_index)
            {
                ...
                output
                    ...
                    << "          \"absolutePositionToleranceM\": " << row.configuration.absolute_position_tolerance_m << ",\n"
                    << "          \"absoluteVelocityToleranceMps\": " << row.configuration.absolute_velocity_tolerance_mps << ",\n"
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        for (const auto& definition : scenario_definitions())
        {
            ...
            scenarios_passed = scenarios_passed && report.production_budget_passed && report.half_tolerance_display_passed && report.tighter_solution_improved;
        ...
        write_report(report_path, passed, maximum_production_error, maximum_half_change, analytic_passed, analytic_rows, reports);
        if (!passed)
        {
            std::cerr << std::setprecision(17) << "adaptive convergence failed: maximum production errors position=" << maximum_production_error.position_m
                      << " m, velocity=" << maximum_production_error.velocity_mps << " m/s, time=" << maximum_production_error.time_s << " s\n";
            for (const auto& row : analytic_rows)
            {
                std::cerr << "analytic h=" << row.maximum_step_s << " speedError=" << row.speed_error_mps << " timeError=" << row.time_error_s << " speedOrder=" << row.observed_speed_order
                          << " timeOrder=" << row.observed_time_order << '\n';
            }
            return 1;
        }
