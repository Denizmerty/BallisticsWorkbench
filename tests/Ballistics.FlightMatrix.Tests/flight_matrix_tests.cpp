...
namespace
{
    ...
    struct FlightResiduals
    {
        ...
        void include(const FlightResiduals& other)
        {
    ...
    ballistics::Vec3 read_vector(const Value::Object& object)
    {
        return {
    ...
    double vector_distance(const ballistics::Vec3& left, const ballistics::Vec3& right)
    {
        return std::hypot(left.x - right.x, left.y - right.y, left.z - right.z);
    }

    ballistics::DragModel read_drag_model(std::string_view value)
    {
        if (value == "G1")
        {
    ...
    ballistics::Projectile read_projectile(const Value::Object& object)
    {
        ...
        {
            projectile.definition.drag = ballistics::SphereDrag { number_field(object, "sphereDiameterM"), number_field(object, "materialDensityKgM3") };
            projectile.provenance.drag_description = "sphere";
        }
        else if (model == ballistics::DragModel::tabulated_cd)
        {
            projectile.definition.drag = ballistics::TabulatedDrag { number_field(object, "dragReferenceDiameterM"), std::move(points) };
            projectile.provenance.drag_description = "tabulatedCd";
        }
        else
        {
            const auto curve = model == ballistics::DragModel::g7 ? ballistics::ReferenceDragCurve::g7 : ballistics::ReferenceDragCurve::g1;
            projectile.definition.drag = ballistics::ReferenceBcDrag { curve, ballistics::ConstantBallisticCoefficient { number_field(object, "ballisticCoefficient") } };
    ...
    ballistics::Atmosphere read_atmosphere(const Value::Object& object)
    {
        const auto atmosphere = ballistics::Atmosphere::create(number_field(object, "temperatureC"),
        ...
        if (std::abs(atmosphere.density_kg_m3 - number_field(object, "densityKgM3")) > transcription_tolerance ||
            std::abs(atmosphere.speed_of_sound_mps - number_field(object, "speedOfSoundMps")) > transcription_tolerance ||
            std::abs(atmosphere.dynamic_viscosity_pa_s - number_field(object, "dynamicViscosityPaS")) > transcription_tolerance)
        {
    ...
    FlightTolerances read_tolerances(const Value::Object& object)
    {
        return {
    ...
    bool within(const FlightResiduals& residuals, const FlightTolerances& tolerances)
    {
        return residuals.position_m <= tolerances.position_m && residuals.velocity_mps <= tolerances.velocity_mps && residuals.time_s <= tolerances.time_s &&
            residuals.ground_speed_mps <= tolerances.scalar && residuals.airspeed_mps <= tolerances.scalar && residuals.mach <= tolerances.scalar &&
            residuals.drag_coefficient <= tolerances.drag_coefficient;
    }

    FlightResiduals compare_sample(const ballistics::TrajectorySample& actual, const Value::Object& expected, const ballistics::Projectile& projectile, const ballistics::Atmosphere& atmosphere)
    {
        ...
        const auto reference_state_diagnostics = ballistics::aerodynamic_diagnostics(projectile, expected_velocity, atmosphere);
        return {
    ...
    std::string trajectory_termination(ballistics::TrajectoryTermination value)
    {
        switch (value)
        {
    ...
    std::string zeroing_status(ballistics::ZeroingStatus value)
    {
        switch (value)
        {
    ...
    std::string mpbr_status(ballistics::MpbrStatus value)
    {
        switch (value)
        {
    ...
    ScenarioReport evaluate_scenario(const Value::Object& scenario)
    {
        ...
        const auto trajectory = ballistics::integrate_trajectory(projectile, atmosphere, maximum_distance, 1.0, 0.25, configuration);
        ...
        report.passed = report.passed && trajectory.termination == ballistics::TrajectoryTermination::requested_distance && within(report.residuals, tolerances);
        return report;
    }

    ZeroingReport evaluate_zeroing_case(const Value::Object& item)
    {
        ...
        const auto zeroed = ballistics::integrate_zeroed_trajectory(projectile, atmosphere, maximum_distance, zero_range, sight_height, 1.0, 0.25);
        report.status = zeroing_status(zeroed.status);
        report.bore_angle_residual_rad = std::abs(zeroed.bore_elevation_rad - number_field(item, "boreElevationRad"));
        ...
        for (const auto& sample_value : samples)
        {
            ...
            report.path_residual_m = std::max(report.path_residual_m, std::abs(actual->position_m.y - sight_height - number_field(expected, "sightPathM")));
            report.time_residual_s = std::max(report.time_residual_s, std::abs(actual->time_s - number_field(expected, "timeS")));
            report.velocity_residual_mps = std::max(report.velocity_residual_mps, vector_distance(actual->ground_velocity_mps, read_vector(object_field(expected, "groundVelocityMps"))));
        }

        const auto& tolerances = object_field(item, "tolerances");
        report.passed = report.passed && zeroed.status == ballistics::ZeroingStatus::complete && report.bore_angle_residual_rad <= number_field(tolerances, "boreAngleAbsoluteRad") &&
            report.path_residual_m <= number_field(tolerances, "pathAbsoluteM") && report.time_residual_s <= number_field(tolerances, "timeAbsoluteS") &&
            report.velocity_residual_mps <= number_field(tolerances, "velocityAbsoluteMps");
        return report;
    }

    MpbrReport evaluate_mpbr_case(const Value::Object& item)
    {
        MpbrReport report;
        report.id = string_field(item, "id");
        const auto projectile = read_projectile(object_field(item, "projectile"));
        ...
        const auto mpbr = ballistics::compute_native_mpbr(projectile, atmosphere, maximum_distance, vital_zone, sight_height, 1.0, 0.25);
        ...
        configuration.terminate_at_ground = false;
        const auto reference_angle_trajectory = ballistics::integrate_trajectory(projectile, atmosphere, maximum_distance, 1.0, 0.25, configuration);
        ...
        const auto& tolerances = object_field(item, "tolerances");
        report.passed = mpbr.status == ballistics::MpbrStatus::complete && report.zero_residual_m <= number_field(tolerances, "zeroAbsoluteM") &&
            report.mpbr_residual_m <= number_field(tolerances, "mpbrAbsoluteM") && report.maximum_path_residual_m <= number_field(tolerances, "maximumPathAbsoluteM");
        return report;
    }
    ...
    void write_residuals(std::ostream& output, const FlightResiduals& residuals, std::string_view indentation)
    {
    ...
    void write_report(const std::filesystem::path& path, std::string_view artifact_id, bool passed, const FlightResiduals& aggregate, std::size_t sample_count, const std::set<std::string>& categories,
        const std::vector<ScenarioReport>& scenarios, const std::vector<ZeroingReport>& zeroing_cases, const std::vector<MpbrReport>& mpbr_cases)
    {
...
int main(int argc, char** argv)
{
    try
    {
        std::filesystem::path scenario_path = std::filesystem::path(BALLISTICS_SOURCE_DIR) / "validation" / "scenarios" / "independent-flight-matrix.json";
        std::filesystem::path report_path = std::filesystem::current_path() / "validation" / "flight-matrix-residuals.json";
        for (int index = 1; index < argc; ++index)
        {
            ...
            {
                throw std::runtime_error("usage: flight_matrix_tests [--scenario PATH] [--report PATH]");
            }
        }

        const auto root = ballistics::json::parse(read_text(scenario_path)).as_object();
        if (number_field(root, "schemaVersion") != 1 || string_field(root, "modelVersion") != ballistics::model_version)
        {
        ...
        write_report(report_path, artifact_id, passed, aggregate, sample_count, categories, scenario_reports, zeroing_reports, mpbr_reports);

        for (const auto& report : scenario_reports)
        {
            std::cout << report.id << ": " << (report.passed ? "PASS" : "FAIL") << " position=" << report.residuals.position_m << " velocity=" << report.residuals.velocity_mps
                      << " time=" << report.residuals.time_s << '\n';
        }
        for (const auto& report : zeroing_reports)
        {
            std::cout << report.id << ": " << (report.passed ? "PASS" : "FAIL") << " bore-angle=" << report.bore_angle_residual_rad << " path=" << report.path_residual_m << '\n';
        }
        for (const auto& report : mpbr_reports)
        {
            std::cout << report.id << ": " << (report.passed ? "PASS" : "FAIL") << " zero=" << report.zero_residual_m << " mpbr=" << report.mpbr_residual_m
